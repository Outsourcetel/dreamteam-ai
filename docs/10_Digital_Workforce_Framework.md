...
| §3.18 | Cost Profile | **Live** | Per-DE token usage + tenant budget enforcement (094/100); FTE Equivalent and ROI now computed live via `get_de_economics()` (migration 131) — strictly downstream of tenant-configured baselines, never stored on the DE row (the old `fte_equivalent`/`roi_score` columns stay dead by design: computed values must never go stale). |
...
| §12 | Workforce Economics (FTE, ROI, executive reporting) | **Live (§12.2–12.4), Partial (§12.5–12.6)** | Migration 131 (2026-07-11): `workforce_baselines` (tenant-configured per-task-type human minutes + monthly fully-loaded FTE cost — every field nullable, never defaulted) + `get_de_economics()` implementing §12.3–12.4's formulas verbatim (9,600 std minutes/month, prorated) over real work counts (inbox decisions, executed actions, conversations) and real windowed AI spend. §12.3's own honesty rule enforced structurally: unconfigured → NULL with the missing baselines named ("configure to calculate"), an unconfigured task type is excluded and named, never silently included. §12.5: tenant-level budget enforcement is real (100); per-DE budget allocation not built. §12.6 report suite not built — `get_de_economics(p_de_id => null)` is the workforce-wide data layer it would read from. |
...
| §16 | Immutable Principles | *Doctrine* | Principle 3 ("trust is earned") is **true today** — per-DE granularity since migrations 108/125. Principle 9 ("incidents are managed") is **true** since migration 123 (durable Incident Record with review lifecycle). Principle 11 ("FTE Equivalent is auditable") is **true** since migration 131 — FTE/ROI compute only from tenant-typed baselines over real counted work, with the calculation inputs returned alongside every result. |
