...
| §8 | Lifecycle (governance gates) | **Partial** | `lifecycle_status` is now a genuine gate for one transition: migration 112 drives Active↔Improving from real open/closed Development Plan items (see §9). Every other of the 12 stages remains a stored label only — no entry/exit criteria enforced for Designed→Configured→Trained→Tested→Certified→Published→Assigned, and 'retired' (migration 110) is still the only other real gate. |
| §9 | Development Framework | **Partial (evidence-grounded slice)** | `de_development_items` (migration 112) is a real Development Plan object — `detect_de_development_needs()` proposes items from real 8-week performance data (escalation rate, confidence, error rate, guardrail-block ratio), not fabricated skill categories; manual items are also supported. Still missing: Skills proficiency profiles (no per-skill granularity exists — `evidence_runs` has no category dimension), Certification records (no reviewer/expiry concept anywhere), and the periodic-scheduled-review trigger (detection is on-demand only, not a cron). |
