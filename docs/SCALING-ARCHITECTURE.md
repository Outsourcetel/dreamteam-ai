...
### Declarative Adapter Framework (shipped — migration 028): rungs 3–4 collapse into DATA
Connecting ANY REST system is now configuration, not code. An **adapter template** (`adapter_templates`) is a jsonb document declaring: the **auth recipe** (api_key_header / bearer / basic / oauth2_client_credentials / none — secret VALUES always stay in `connector_secrets`), the **base-URL shape** with per-connector variables (`https://{subdomain}.example.com/api/v2`), and per-category-op **HTTP bindings** ({query}/{ref}/{var} templating in path, query params, JSON body) with **response dot-paths** (items_path/id_path/title_path/snippet_path/url_path). One executor in `connector-hub` (provider `template`) runs every template: render → auth → fetch → walk dot-paths → canonical items, with a structured error for every failure class (`var_missing`, `auth_failed`, `op_not_bound`, `http_NNN`, `path_not_found_in_response` **with the keys actually found** — a non-developer can debug from the builder). The same validator (`_shared/adapterTemplates.ts`, browser mirror `src/lib/adapterTemplates.ts`) runs on save AND on execute. **Scale consequence:** a new named system is a template row, not an adapter PR — the platform library (`scope='platform'`) grows by seeding data, tenant libraries (`scope='tenant'`) grow through the guided builder with live "Test now" (raw response next to extracted items). The bespoke adapters that remain (Zendesk/Salesforce/Confluence/Jira/Intercom) are the ones needing non-REST idioms (SOQL, CQL, ADF walking) — everything REST-shaped goes through templates.

### Data access grants layer (shipped — migration 029): default-deny under the category contracts
At scale, "connected" cannot mean "reachable by every DE". The grants layer sits under the category contracts: every machine subject (DE or Specialist) needs an explicit `data_access_grants` row to touch a connector — a CUMULATIVE ladder (search ⊂ read ⊂ ingest ⊂ write_back), one row per subject × resource holding the max level. Grants target a **category** ("may read any helpdesk system" — survives connector swaps, which is the point of category contracts) or a **specific connector** (beats the category grant). `resolve_access` (SECURITY DEFINER) is the single enforcement primitive; the hub, the evidence pipeline, playbook connector steps and Scribe all call it server-side with the acting subject; no grant = deny + `data_access_denied` audit event. Defaults are seeded per domain (`seed_default_grants`) and NEVER include erp_financials/billing/payroll_hcm. **Scale consequence:** hiring DE #50 doesn't widen the blast radius of connector #200 — access is a governance decision per subject, not a side effect of connecting. **Named upgrade:** per-DE knowledge scopes — internal `knowledge_docs` remain tenant-wide in v1; the same subject/grant pattern extends to knowledge when a tenant needs DE-level knowledge isolation.
