...
| 5 | `de-fitness-measure` + `amendment_metrics` + `fitness_run_progress` (310/312/313/690/695) | Before/after fitness of an applied persona amendment, on a frozen golden set (twenty questions) | Frozen `golden_qa` ids, de-simulate measure mode, both personas back-to-back | `amendmentMetricsApi.ts` → SelfLearningPage; digest delta (309) | Informs adopt/rollback review | **HONEST** — an exam **by declared design** (A/B on the same test), fail-closed NULL/NULL. Population: **1 measured row ever** (08-10); `amendment_fitness.enabled='true'`; cron `de-fitness-measure-driver` live (*/30). Denominator honesty: one measurement is a data point, not a track record. |
