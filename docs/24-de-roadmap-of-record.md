...
- **GI-6 — Make "it gets measurably better" measurable. ⏳ DECISION PENDING.** The naive fix (two time-separated ~4-question golden replays before/after) measures *noise* — non-deterministic LLM + tiny unordered sample + moving baseline — and would render "fitness +X" as improvement dishonestly (honesty-mandate violation). Doing it right = a candidate-persona back-to-back replay (current vs proposed persona on a fixed ordered set, pass-count delta like de-improve) + capture-at-adoption + exclude specialist charter-only edits (invisible to de-answer). A real build; awaiting founder steer.
