...
- **GI-6b — Real amendment fitness (candidate-persona replay). ⏳ NEXT / needs a focused pass.** Add a candidate-persona override to de-answer/de-simulate (like de-improve's candidate_knowledge), run ONE back-to-back replay over a fixed ordered golden set (current vs proposed persona, pass-count delta), capture at adoption (not a poll racing approval), exclude specialist charter-only edits. Touches the core answer path + is honesty-critical → deserves its own design→adversarial-verify pass, not a rushed build.
