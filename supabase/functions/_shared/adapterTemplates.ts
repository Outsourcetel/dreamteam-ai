 ...
 * (single source for edge functions; mirrored for the browser at
 * src/lib/adapterTemplates.ts — keep the two files in sync.)
 *
...
import { SystemCategory, CATEGORY_OPS, legalOps } from './categoryContracts.ts';
 ...
 * a concrete { method, url, headers-less body } given the connector's
 * base URL, its template variables, and the caller's params. Used by
 * BOTH preview_action (render only, never call out) and execute_action
 * (render, then actually fetch) so preview and execute can never drift
 * — the exact same function produces the exact same request shape.
 */
