-- 815_no_evidence_and_not_looked_at_are_different.sql
-- ==========================================================================
-- WHAT THE LADDER COULD NOT SAY.
--
-- Promotion needs decided human reviews. trust_evidence_for counts only
-- human_tasks in (approved, rejected) with a decided_at inside the window, so
-- an employee that escalates correctly and is ignored accumulates NOTHING,
-- and is indistinguishable from one that has done nothing at all. Both read:
--
--   human_samples         0 decided reviews (needs 5)
--   human_approval_rate   0 of 0 human reviews approved in the last 30 days
--
-- That second line is the one that misleads. A rate over zero observations
-- is not 0%, it is absent — but "0 of 0 approved" reads as an employee that
-- scored nothing. Measured on production: Accounting DE shows exactly that,
-- while holding 23 undecided escalations, every one of which it was right to
-- raise. It refused to declare a ledger clear it could not read.
--
-- ── WHAT THIS DOES NOT DO ─────────────────────────────────────────────────
-- It does not change what earns autonomy. Not one threshold moves, not one
-- `met` flips, and nothing becomes promotable that was not promotable
-- before. Requiring evidence before trust is correct, and an unanswered
-- escalation is genuinely not evidence of good judgment — only of judgment
-- nobody has checked.
--
-- Whether a CORRECT REFUSAL should itself count toward trust is a real
-- product question with money and authority attached, and it is the
-- founder's to answer, not a default for a migration to invent. Mig 765
-- settled the same shape the same way: judging everyone against three
-- invented constants was the defect, not the fix.
--
-- ── WHAT IT DOES ──────────────────────────────────────────────────────────
-- Makes the evidence tell the truth about WHY the sample size is zero, and
-- carries the number so a reader need not parse prose:
--
--   before   0 of 0 human reviews approved in the last 30 days
--   after    no reviews decided in the last 30 days — 23 awaiting a decision
--
-- The pending count uses the same tenant, window and production-origin
-- filter as the decided count, so the two numbers are comparable rather than
-- two different questions with similar names.
-- ==========================================================================
...
begin
  ...
  v_criteria := jsonb_build_array(
    ...
    jsonb_build_object(
      ...
      'detail', format('%s decided reviews (needs %s)%s', v_h_total, v_min_h_n,
...
do $verify$
declare
  v_pol trust_policies; v_ev jsonb; v_rate_detail text; v_n_detail text;
  v_before_met jsonb; v_after_met jsonb;
begin
  -- A policy whose employee has undecided reviews and no decided ones — the
  -- exact case this migration exists for. Without one, everything below
  -- measures nothing.
  select tp.* into v_pol
    from trust_policies tp
   where tp.status = 'active'
     and exists (select 1 from human_tasks t
                  where t.tenant_id = tp.tenant_id and t.de_id = tp.de_id
                    and t.status = 'pending')
     and not exists (select 1 from human_tasks t2
                  where t2.tenant_id = tp.tenant_id and t2.de_id = tp.de_id
                    and t2.status in ('approved','rejected')
                    and t2.decided_at >= now() - interval '30 days')
   limit 1;
  if v_pol.id is null then
    raise exception 'VERIFY FAILED: no policy with pending-but-undecided reviews, so nothing below is measured';
  end if;

  v_ev := public.trust_evidence_for(v_pol);

  -- (a) the count is carried, and it is not zero for this policy
  if coalesce((v_ev->>'pending_reviews')::bigint, 0) = 0 then
    raise exception 'VERIFY FAILED: pending_reviews is 0 for a policy chosen because it HAS pending reviews';
  end if;

  -- (b) ⛔ THE POINT. The approval-rate line must no longer claim a score.
  select c->>'detail' into v_rate_detail
    from jsonb_array_elements(v_ev->'criteria') c where c->>'key' = 'human_approval_rate';
  if v_rate_detail like '0 of 0%' then
    raise exception 'VERIFY FAILED: approval rate still reads as a 0%% score: %', v_rate_detail;
  end if;
  if v_rate_detail !~ 'awaiting a decision' then
    raise exception 'VERIFY FAILED: approval rate does not say how many are waiting: %', v_rate_detail;
  end if;

  -- (c) and the sample-size line carries it too
  select c->>'detail' into v_n_detail
    from jsonb_array_elements(v_ev->'criteria') c where c->>'key' = 'human_samples';
  if v_n_detail !~ 'awaiting a decision' then
    raise exception 'VERIFY FAILED: sample size does not say how many are waiting: %', v_n_detail;
  end if;

  -- (d) ⛔ AND NOTHING BECAME PROMOTABLE. This is a reporting change; if any
  --     `met` or the overall eligibility moved, it is a policy change wearing
  --     a reporting change's clothes.
  if coalesce((v_ev->>'eligible')::boolean, false) then
    raise exception 'VERIFY FAILED: a policy with zero decided reviews is now ELIGIBLE — thresholds moved';
  end if;
  select jsonb_agg(c->>'met' order by c->>'key') into v_after_met
    from jsonb_array_elements(v_ev->'criteria') c
   where c->>'key' in ('human_samples','human_approval_rate');
  if v_after_met <> '["false","false"]'::jsonb then
    raise exception 'VERIFY FAILED: human criteria met-flags are %, expected both false', v_after_met;
  end if;
end
$verify$;
