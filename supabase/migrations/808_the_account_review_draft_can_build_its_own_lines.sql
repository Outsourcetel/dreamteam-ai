-- 808_the_account_review_draft_can_build_its_own_lines.sql
-- ============================================================================
-- account_review_draft() IS BROKEN IN PRODUCTION RIGHT NOW. Not a hazard, not
-- a style finding — every call throws:
--
--     22P02  malformed array literal
--
-- Proven before writing this, by calling it against a real customer_accounts
-- row inside a rolled-back transaction. Every account review draft fails.
--
-- ── The cause, which this repo has already paid for twice ──────────────────
-- `v_lines` is text[], and mig 806 appends to it with `v_lines || <literal>`.
-- Against an UNKNOWN-typed literal that operator resolves to anyarray||anyarray
-- rather than array_append, so Postgres tries to parse the sentence AS AN ARRAY
-- LITERAL and throws 22P02.
--
-- Migration 685 fixed exactly this in validate_onboarding_items and 693 wrote
-- the reason down; 741 then reintroduced it 36 times, and scripts/
-- migration-append-check.mjs exists because "a fact that lives only in one
-- migration's comment is a fact the next author will not have". 806 is the
-- next instance, and this is the third time.
--
-- ── Why a new migration and not an edit to 806 ─────────────────────────────
-- 806 is applied (2026-08-20 01:00:32). public.schema_migrations records a
-- checksum taken at apply time, so editing the file would make migrate:status
-- report DRIFTED — a true statement that would then be a lie about the cause.
-- An applied migration's file must keep describing what actually ran. Same
-- shape as 685: fix it FORWARD.
--
-- ── What changed, and nothing else ─────────────────────────────────────────
-- The body below is 806's function AS IT IS LIVE, with all NINE
-- `v_lines := v_lines || X;` statements rewritten to
-- `v_lines := array_append(v_lines, X);` — transformed mechanically by scanning
-- for the statement's terminating semicolon at paren-depth zero and outside
-- string literals, so a `;` inside format() or inside a sentence could not be
-- mistaken for the end. array_append is unambiguous whatever the operand's type
-- resolution, which `::text` on each literal would also have achieved — but
-- only for the literals somebody remembered to cast, and two of the nine were
-- already cast and still left the function throwing.
-- ============================================================================
...
begin
  ...
  if v_measured then
    v_lines := array_append(v_lines, format('Health: %s (%s). Penalties — open tickets %s, escalations %s, overdue invoices %s, activity recency %s.',
    ...
    v_lines := array_append(v_lines, format('Health: NOT MEASURED — no tickets, invoices or activity are recorded for this account yet, so there is nothing to compute a score from. The score on file (%s) is whatever was entered by hand.',
      coalesce(v_acct.health_score::text, 'none')));
  end if;

  v_lines := array_append(v_lines, format('Receivables: %s open invoice(s)%s%s.',
    n_open_inv,
  ...
  v_lines := array_append(v_lines, case
  ...
  if p_kind = 'health_check' then
    ...
    v_lines := array_append(v_lines, 'Approve to file this review. Reject with a note if something here needs a different owner or a correction first.'::text);
    ...
    if v_days_to_renewal is not null and v_days_to_renewal between 0 and 90 and v_case.stage_key is null then
      v_lines := array_append(v_lines, 'The renewal is inside 90 days and no continuity case is open — opening one is the concrete next step.'::text);
    end if;
    v_lines := array_append(v_lines, 'No score history is kept yet; this review records today''s position so next quarter has something to compare against.'::text);
...
-- ── Proof ───────────────────────────────────────────────────────────────────
-- Absence of a violation, per CLAUDE.md rule 3: no bare append survives in the
-- installed definition. Vacuously safe on any environment and still catches a
-- reintroduction. Deliberately NOT a live call — that would need a
-- customer_accounts row and make this migration unreplayable, which is the
-- defect the same rule exists to prevent.
do $$
declare v_def text;
begin
  select pg_get_functiondef(oid) into v_def
    from pg_proc where oid = 'public.account_review_draft(text, uuid)'::regprocedure;

  if v_def ~ 'v_lines\s*\|\|' then
    raise exception '808: account_review_draft still appends to v_lines with ||, which throws 22P02 on an unknown-typed literal. Use array_append().';
  end if;

  if v_def !~ 'array_append\(v_lines' then
    raise exception '808: account_review_draft no longer builds v_lines with array_append — the rewrite did not survive.';
  end if;
end $$;
