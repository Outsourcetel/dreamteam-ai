-- 586 — an employee excused from the exam was being failed for not sitting it.
--
-- trust_evidence_for scores five criteria. Two of them are "rate" checks with
-- a companion "sample size" check, and they were written differently:
--
--   human_approval_rate : met = (v_min_h_n = 0 or (samples >= min and rate >= bar))
--   eval_pass_rate      : met = (samples >= min and rate >= bar)
--
-- The human one honours a minimum of zero as "not applicable". The eval one
-- does not — so a policy setting min_eval_samples = 0 got a rate of 0 measured
-- against a bar of 0.9, and could never pass however well the employee worked.
--
-- That is exactly the state Finance DE reached today. After the founder worked
-- the approvals queue it had human_samples 4 against 3, human_approval_rate
-- met, guardrail_blocks met, eval_samples 0 against a required 0 — and was
-- still ineligible on an exam its own policy says it does not take.
--
-- This is NOT a lowered bar. No threshold moved. It makes one criterion
-- respect a setting the policy already carries, the way its sibling already
-- does. If action-executing employees SHOULD be examined on answering, the fix
-- is to set min_eval_samples above zero on those policies — not to leave a
-- criterion that cannot be satisfied.
