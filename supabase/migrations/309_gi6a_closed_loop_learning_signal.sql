-- 309_gi6a_closed_loop_learning_signal.sql
-- ============================================================================
-- GI-6a (Governance Integrity) — surface a REAL self-improvement signal now.
--
-- The amendment "fitness" number (fitness_avg_delta) is not yet trustworthy —
-- measuring it honestly needs a candidate-persona back-to-back replay (GI-6b).
-- Until then the fitness chip stays hidden (it renders nothing today anyway,
-- since fitness_samples = 0). But the workforce genuinely DOES have a real,
-- honest "it gets measurably better" signal already in the data: the CLOSED
-- LOOP on recurring problems — knowledge_gap_clusters records fix_applied_at,
-- recurred_after_fix and recurrence_count, so we can honestly report:
--   • recurring issues fixed in the window (recurrence_count >= 1 + fixed)
--   • how durable fixes are all-time (held vs reopened) — the compounding-moat
--     evidence, with an honest denominator (shown only with real sample).
--
-- Reproduces get_workforce_learning_digest VERBATIM from mig 256:22-117 (the ONLY
-- definition; no later redefinition) and adds ONE new 'learning' block; every
-- existing block is byte-identical. GLOBAL.
-- ============================================================================
