-- 760_a_topic_decides_who_answers.sql
-- ==========================================================================
-- WHY: migration 754 gave "conversation topics" a real writer and then wrote,
-- on the card and in its own header, that a topic LABELS and does not ROUTE.
-- That was true, it was measured, and the founder's answer was to make the
-- other half true rather than to keep the disclaimer. This is the ROUTING
-- half of the ruling "1 and 3": A CONVERSATION TOPIC DECIDES WHO ANSWERS.
--
-- THE THREE DECISIONS THIS FILE IS BUILT TO, ALL SETTLED BEFORE IT WAS WRITTEN
--
--  1. CLASSIFY THE QUESTION ONLY, and use that ONE result for BOTH the routing
--     and the stored label.
--  2. THE TOPIC OWNER COMES FROM BOTH — the interview SEEDS it, the
--     Support › Triage rules screen EDITS it. A customer must be able to
--     change their mind on the screen without re-running the interview.
--  3. NO MATCH KEEPS TODAY'S BEHAVIOUR EXACTLY. Purely additive.
--
-- ==========================================================================
-- ⚠⚠⚠ DECISION 1, AND THE LIVE PROOF THAT IT IS A BUG FIX AND NOT A
-- PREFERENCE.
--
-- `trg_triage_support_conversation` classifies
-- `coalesce(subject,'') || ' ' || coalesce(NEW.content,'')`, and widget-ask
-- stores the first message as `[channel · endUserTag] question` where
-- endUserTag is the END USER'S DISPLAY NAME and account ref
-- (widget-ask/index.ts:319, :383). `classify_support_text` matches BARE
-- SUBSTRINGS with `position()` — the live baseline patterns include fragments
-- as short as `fire`, `mfa` and `invoice` — and Safety runs at rule_order 10,
-- ahead of Billing at 60.
--
-- Run live against a real tenant's real rules on 2026-08-18:
--
--   classify('what are the payment terms on my invoice?')
--       -> {"rule":"Billing","category":"billing","priority":"normal","severity":"sev3"}
--   classify('[widget · Fireside Media · account FIRE-1] what are the payment
--             terms on my invoice?')
--       -> {"rule":"Safety","category":"safety","priority":"urgent","severity":"sev1"}
--
-- An account called "Fireside Media" asking about payment terms is filed as a
-- SAFETY EMERGENCY because `fire` is a substring of its name. Today that is a
-- wrong label on an inbox row. Under routing it would be A COMPANY NAME
-- DECIDING WHO ANSWERS. So the classification moves onto the question alone,
-- and the same single result is used for the routing and for the label — which
-- fixes the existing mislabelling as a side effect rather than leaving it.
--
-- ⚠ AND THE BLAST RADIUS, MEASURED RATHER THAN ASSUMED. Across all 177 live
-- first user messages on triaged channels (widget/hosted/portal/email/dock),
-- comparing what the trigger computes today against what the question alone
-- computes: 0 differ on category, 0 on priority, 0 on severity, 0 on rule.
-- 21 of those messages carry a bracket prefix and 14 carry a subject, so the
-- comparison had something to compare — the live display names are
-- "DE-A2 Verification", "Review Drive Customer", "E2E Tester" and so on, none
-- of which contains a trigger substring. THE COMPARISON CAN FAIL, and the
-- Fireside case above is it failing on demand. Both facts are needed: the
-- second says the check is real, the first says shipping it re-labels nothing
-- that already exists.
--
-- ==========================================================================
-- ⚠⚠ WHY THE CLASSIFICATION MOVES IN FRONT OF THE INSERT, AND WHY de_id IS
-- NEVER RE-STAMPED.
--
-- The obvious shape is to leave the writers alone and re-point de_id after
-- triage. It is wrong, and the reason is attribution. `de_conversations.de_id`
-- is what the whole employee-performance surface counts by:
...
-- compose_weekly_value_digest all read `from de_conversations where de_id =
-- p_de_id and created_at > now() - interval`. Moving de_id mid-flight would
-- retroactively move an ENTIRE conversation — its economics, its CSAT, its
-- performance review, and the messages the FIRST employee actually handled —
-- onto somebody else.
--
-- So the choice is made where it can be made once: BEFORE THE ROW EXISTS.
-- Inbound text -> classify -> look up the topic's owner -> INSERT the
-- conversation with de_id AND all four triage columns already correct.
--
-- ⚠ de_id IS WRITE-ONCE, AND THE REAL REASON IS GRANTS, NOT GREPS. Twelve SQL
-- functions UPDATE de_conversations (approve_draft_reply,
-- claim_support_conversation, handoff_back_to_de, park_support_conversation,
-- send_human_reply, set_support_conversation_state, submit_csat,
-- sync_conversation_draft_decision, trg_support_sentiment,
-- trg_triage_support_conversation, unpark_support_conversation,
-- verify_and_bind_widget_identity) and NOT ONE names de_id in its SET clause —
-- read out of pg_proc, not out of a comment. And `anon` and `authenticated`
-- hold SELECT, REFERENCES and TRIGGER on that table and nothing else, so no
-- browser can re-point a conversation however the UI is built. Probe 19
-- asserts both, both ways.
--
-- ==========================================================================
-- ⚠⚠⚠ THE TRIGGER GUARD HIDES A SILENT REGRESSION, AND IT IS THE SHARPEST
-- EDGE IN THIS FILE.
--
-- `priority` is NOT NULL DEFAULT 'normal'. `severity` and `triaged_at` are
-- nullable with no default. `trg_triage_support_conversation` returns early on
-- `v_conv.category IS NOT NULL` (migration 671). So a routed insert that
-- stamped only `category` would leave severity NULL and priority stuck at
-- 'normal' FOREVER — on exactly the traffic where Safety writes sev1/urgent
-- today. THE ROUTED INSERT MUST WRITE ALL FOUR TOGETHER, and probe 19(e)
-- drives all three shapes against the real trigger to prove it: all four set
-- (unchanged), only category set (severity NULL, priority 'normal' — the
-- regression, driven), and none set (the trigger fills them, which is the
-- vacuity guard for the other two).
--
-- ==========================================================================
-- ⚠⚠ THERE ARE THREE INSERT SITES, NOT TWO, AND THE THIRD MADE MOST OF THE
-- CONVERSATIONS ON THIS PLATFORM.
--
--   · supabase/functions/widget-ask/index.ts:338  — widget | hosted
--   · supabase/functions/email-inbound/index.ts:190 — email
--   · supabase/functions/de-answer/index.ts:584   — exam | portal | dock,
--     with its own oldest-eligible fallback at :493-507. It created 446 of the
--     460 conversations that exist. Leaving it out would be two paths and one
--     counted.
--
-- de-answer is wired TIGHTLY: routing fires only when the caller NAMED NOBODY
-- and the channel is triaged. The dock passes an explicit de_id
-- (knowledgeApi.ts:640, DEChatDock.tsx:355 — the Workspace Assistant), so the
-- workspace's own teammate chat is untouched. The path that actually reaches
-- the new code is the CUSTOMER PORTAL: EndUserChatPage.tsx:190 and :259 pass
-- de_id null with channel 'portal'. `exam` is excluded because it is not in
-- the trigger's channel set either — migration 671 took exams out of the
-- support taxonomy on purpose and this file does not put them back.
--
-- ⚠ external_reply_mode = 'auto' IS ZERO ACROSS ALL 109 EMPLOYEES ON ALL 18
-- TENANTS. The "preferring auto" half of today's fallback HAS NEVER SELECTED
-- ANYTHING — `?? frontDes[0]` resolves every time. So "no match routes exactly
-- as today" is pinned against OLDEST-ELIGIBLE, which is the branch that
-- actually runs; a pin written against the auto branch would be green on zero
-- comparisons.
--
-- ==========================================================================
-- ⚠⚠ THERE ARE TWO ROUTERS ON THIS PLATFORM, AND THIS FILE SAYS HOW THEY
-- RELATE RATHER THAN LEAVING THEM BOTH.
--
-- `de-orchestrate` ALREADY routes (de-orchestrate/index.ts:151-171): a
-- designated SUPERVISOR employee is shown the roster and an LLM picks by
-- RESPONSIBILITY FIT. It is DORMANT — `is_supervisor` is false on all 109
-- employees across all 18 tenants, so it falls straight through to de-answer
-- today — and it wakes the day anyone sets one.
--
-- ⚠⚠⚠ THIS PARAGRAPH SAID THE OPPOSITE UNTIL THE FOUNDER RULED, AND THE OLD
-- VERSION IS LEFT HERE IN ONE SENTENCE SO THE CHANGE IS NOT INVISIBLE: it read
-- "an employee the caller named always wins ... so on a workspace that has a
-- supervisor the supervisor's judgement decides and the topic owner stands
-- down". That made AI-overrides-customer the platform default, silently, on
-- the day anybody first sets is_supervisor.
--
-- THE RULE IS: THE CUSTOMER'S TOPIC OWNER BEATS THE AI ROUTER. An owner the
-- customer explicitly named — in the interview, or on the Support › Triage
-- rules screen — wins, and the router chooses only where no topic matched. One
-- precedence, and it is the same one the widget uses:
--
--   1. an explicitly bound widget key (migration 323), or a caller-supplied
--      de_id, or THE EMPLOYEE THE THREAD ALREADY BELONGS TO — a human named
--      somebody, or a conversation already recorded who it belongs to;
--   2. the TOPIC OWNER — the customer wrote a rule saying who answers this.
--      de-orchestrate consults this BEFORE it asks the model, and skips the
--      model call entirely when a topic matched;
--   3. today's fallback — external_reply_mode='auto' if any, else the oldest
--      eligible employee. This is also where a thread whose recorded owner has
--      since been retired lands: never back at the classifier.
--
-- Said plainly because it is a real consequence: on a workspace that sets a
-- supervisor, a NEW dock or portal thread whose question matches an owned
-- topic goes to that owner and the supervisor's model is never called; where
-- no topic matches, the supervisor routes exactly as it does today. Widget and
-- email never reach de-orchestrate at all.
--
-- ⚠ STILL OPEN, AND NOT CLOSED BY THIS FILE: de-orchestrate can move who
-- answers MID-THREAD (it passes `de_id: chosen` with the caller's
-- conversation_id, by design, so a teammate answers on the same thread for
-- shared memory) while de_conversations.de_id stays where it was. That is the
-- same attribution gap the three writers just closed, on the one path the
-- founder's ruling did not name — and it is unreachable until somebody sets a
-- supervisor.
--
-- ==========================================================================
-- ⚠ WHAT THIS FILE DOES NOT DO, stated so it is not discovered later.
--
--  · It does not re-route conversations that already exist. de_id is chosen at
--    INSERT and never moved (above). The 460 conversations on this platform
--    keep the employee they have. ⚠ The FIRST version of the TypeScript half
--    did not honour that: classifyAndRoute was called before the
--    conversation-reuse check on all three writers, so turn 2 of an open thread
--    was re-classified and could be answered, charged and escalated by a
--    different employee while de_id still named the first. Fixed in the same
--    round as this note — a reused thread is not classified at all, and the
--    employee is whoever the ROW says.
--  · It does not let a topic set priority or severity from the interview. A
--    model still cannot choose rule_order, set_priority, set_severity or the
--    tenant; createTriageRuleFromProposal writes the column defaults. What it
--    can now choose is the OWNER, and that is the founder's decision 2.
--  · It does not touch `exam` conversations, or any channel outside the five
--    trg_triage_support_conversation already accepts.
--
-- ==========================================================================
-- WHAT THIS FILE DOES, IN ORDER
--   PART 1 — digital_employees gains the composite key a same-tenant foreign
--            key needs to point at. Argued, not slipped in.
--   PART 2 — support_triage_rules.owner_de_id, its FK and its index.
--   PART 3 — classify_support_text returns the matched rule's id and its
--            ELIGIBLE owner, so ONE call answers both questions.
--   PART 4 — decide_discovery_proposal's conversation_type branch learns the
--            owner, and `routes_to_employee` becomes an expression.
--   PART 5 — the standing check: probe 18's pin re-stated, PROBE 19 added.
...
-- ===========================================================================
-- PART 1 — THE FOREIGN-KEY TARGET, ARGUED RATHER THAN SLIPPED IN.
--
-- `owner_de_id` has to be a SAME-TENANT reference. A plain
-- `references digital_employees(id)` would let a workspace point a triage rule
-- at ANOTHER workspace's employee: RLS on support_triage_rules bounds the
-- tenant of the ROW, not of the row it references, and "who answers" resolved
-- through a tenant-id the writer chose is precisely the shape migrations
-- 662-664 exist to prevent.
--
-- The composite form `(tenant_id, owner_de_id) -> digital_employees(tenant_id,
-- id)` puts that in the database. It needs a unique key over (tenant_id, id),
-- and digital_employees does not have one (read live: pkey on id, plus two
-- PARTIAL uniques on tenant_id for the workspace assistant and the supervisor).
--
-- ⚠ SO WHAT DOES ADDING IT COST? Nothing that can refuse a write. `id` is
-- already unique on its own, so (tenant_id, id) is unique for free — this
-- constraint CANNOT reject any row the primary key would have admitted. It is
-- a pure reference target. The only behaviour it adds is that `on conflict
-- (tenant_id, id)` becomes resolvable, and a conflict there is a conflict on
-- the primary key by definition. 109 rows today.
--
-- THE ALTERNATIVES, AND WHY EACH LOSES:
--   · plain FK on (id) + a tenant filter in the lookup — the write succeeds and
--     the topic then silently stops routing. A refused write is a message; a
--     rule that quietly does nothing is the failure mode this whole workstream
--     keeps finding.
--   · a trigger enforcing same-tenant — a THIRD statement of the rule in a
--     third language, free to drift, and this repo has paid for a check that
--     lived beside the thing it was supposed to gate.
--   · no FK at all — a deleted employee leaves a dangling uuid that the Triage
--     screen renders as a broken owner and nothing ever cleans up.
-- ===========================================================================
alter table public.digital_employees
  drop constraint if exists digital_employees_tenant_id_id_uq;

alter table public.digital_employees
  add constraint digital_employees_tenant_id_id_uq unique (tenant_id, id);

comment on constraint digital_employees_tenant_id_id_uq on public.digital_employees is
  'A reference target, not a rule. `id` is already unique by itself, so this can refuse nothing the primary key would have admitted — it exists so support_triage_rules.owner_de_id can be a SAME-TENANT foreign key (migration 760) instead of a plain reference to an id plus a tenant filter somebody could delete. '
  'Changing or dropping it breaks support_triage_rules_owner_de_fk, which is what stops one workspace pointing a triage rule at another workspace''s employee.';


-- ===========================================================================
-- PART 2 — support_triage_rules LEARNS WHO ANSWERS.
--
-- Nullable, and NULL is the meaningful default: it is "the workspace's usual
-- choice", which is exactly today's behaviour and the founder's decision 3.
-- All 198 live rules across all 18 tenants carry NULL after this migration,
-- so nothing routes differently until somebody says so.
--
-- ⚠⚠ ON DELETE SET NULL (owner_de_id) — AND THE COLUMN LIST IS LOAD-BEARING.
-- A composite `on delete set null` WITHOUT it nulls EVERY referencing column,
-- tenant_id included, and tenant_id is NOT NULL — so the delete raises 23502
-- and the constraint behaves as RESTRICT: an employee nobody could ever remove
-- because a triage rule points at them. The column list is Postgres 15+ and
-- this database is 17.6 (read live). Probe 19(g) drives the delete and asserts
-- BOTH halves: it succeeds, and the rule survives with its tenant intact.
--
-- The three choices, each rejected for a reason a customer would feel:
--   · CASCADE would DELETE THE CUSTOMER'S TOPIC because an employee left. The
--     rule is theirs; the person is not.
--   · RESTRICT (or the accidental RESTRICT above) would make an employee
--     undeletable.
--   · SET NULL leaves the topic working and unowned, which is the same state
--     it starts in and the one the Triage rules screen can fix.
--
-- ⚠ ON UPDATE is left at NO ACTION deliberately. Moving an employee to a
-- different tenant while a rule points at them should fail loudly; nothing on
-- this platform does it.
-- ===========================================================================
alter table public.support_triage_rules
  add column if not exists owner_de_id uuid;

alter table public.support_triage_rules
  drop constraint if exists support_triage_rules_owner_de_fk;

alter table public.support_triage_rules
  add constraint support_triage_rules_owner_de_fk
  foreign key (tenant_id, owner_de_id)
  references public.digital_employees (tenant_id, id)
  on delete set null (owner_de_id);

-- The referencing side of a foreign key wants its own index or every DELETE on
-- digital_employees seq-scans this table. 198 rows makes it free today and
-- cheap forever; an unindexed FK is a footgun that only shows up under load.
create index if not exists support_triage_rules_owner_de_idx
  on public.support_triage_rules (owner_de_id)
  where owner_de_id is not null;

comment on column public.support_triage_rules.owner_de_id is
  'WHO ANSWERS conversations this rule matches, or NULL for "the workspace''s usual choice" — which is what every one of the 198 rules that existed before migration 760 carries, and is byte-for-byte today''s behaviour. '
  'Read by classify_support_text, which returns it ONLY when the employee is still eligible (same tenant, not the Workspace Assistant, lifecycle not paused/retired/archived/designed) — so a retired owner falls back rather than routing to somebody who no longer works here. Rows are retired on this platform, never deleted, which is why the filter is on lifecycle_status and not on existence. '
  'Written from two places by design (the founder''s decision, 2026-08-18): the discovery interview SEEDS it through decide_discovery_proposal''s conversation_type branch, and the Support › Triage rules screen EDITS it — a customer changes their mind on the screen, never by re-running the interview. '
  '⚠ The foreign key is COMPOSITE over (tenant_id, owner_de_id) so the reference cannot cross a workspace boundary, and ON DELETE SET NULL carries the Postgres 15 column list so a departing employee clears the owner instead of nulling tenant_id (which would raise 23502 and make the employee undeletable) or cascading (which would delete the customer''s topic).';

-- No GRANT is issued. `authenticated` already holds column-less
-- INSERT/UPDATE/DELETE/SELECT on this table, which covers a new column, and
-- support_triage_rules_admin_write (FOR ALL over owner/admin/manager with
-- tenant_id = auth_tenant_id()) is what bounds the write. That is the same bar
-- that already governs editing a rule's pattern or category — a manager who
-- can re-file every conversation in the workspace by editing a pattern is not
-- newly dangerous for being able to say who answers it — and it is the screen
-- the founder named.


-- ===========================================================================
-- PART 3 — classify_support_text ANSWERS BOTH QUESTIONS AT ONCE.
--
-- ⚠⚠ ONE CLASSIFICATION, ONE ANSWER — the founder's decision 1, expressed as a
-- function signature rather than as a convention. The alternative was a second
-- lookup keyed on the CATEGORY, and it is wrong in a way that would not show
-- up for months: `classify_support_text` returns on the FIRST matching rule
-- ordered by (rule_order, created_at), and nothing stops two rules sharing a
-- set_category. A category-keyed lookup would sometimes resolve the owner off
-- a DIFFERENT rule from the one that produced the label — two paths, one
-- counted. The owner therefore comes off the row that matched, which means it
-- has to come out of this function.
--
-- ⚠ WHAT CHANGES FOR EXISTING CALLERS: nothing. The four keys that were there
-- (category, priority, severity, rule) keep their exact meanings and values;
-- `rule_id` and `owner_de_id` are added. PART 6 proves that by re-running the
-- function over EVERY live first user message on a triaged channel and
-- comparing the four-key answer before and after, byte for byte.
--
-- ⚠ THE PERIMETER IS ALREADY RIGHT, and this was measured rather than assumed:
-- migration 233 granted it to `authenticated`, and migration 662 REVOKED that.
-- Read live 2026-08-18: anon NO, authenticated NO, public NO, service_role YES.
-- So returning an employee id here widens no cross-tenant read — the only
-- callers are the trigger, the verifier and the three edge functions, all of
-- which already hold the service role. `create or replace` keeps the oid and
-- therefore the ACL; the REVOKE/GRANT below restates it rather than changing it.
--
-- ⚠ THE LOOP IS RESTRUCTURED TO ONE RETURN, and that is not tidying. The old
-- body returned from two places; adding an owner lookup to both would have
-- been the same expression written twice, which is how the two copies drift.
-- `EXIT rules` carries the matched row out and everything after it happens
-- once.
-- ===========================================================================

-- The BEFORE picture, taken while the old function is still in place. PART 6
-- compares against it. `on commit drop` so it cannot outlive this transaction
-- in either direction — commit or rollback.
create temp table _760_cls_before on commit drop as
  select c.id as conv_id,
         c.tenant_id,
         public.classify_support_text(c.tenant_id,
           coalesce(c.subject, '') || ' ' || coalesce(m.content, '')) as cls
    from public.de_conversations c
    join lateral (
      select mm.content
        from public.de_messages mm
...
comment on function public.classify_support_text(uuid, text) is
  'The ONE deterministic answer the support taxonomy gives about a piece of text: which topic it is, how urgent, how severe, which rule decided — and, since migration 760, WHICH RULE (rule_id) and WHICH EMPLOYEE ANSWERS IT (owner_de_id). '
  'Walks public.support_triage_rules for the tenant ORDER BY rule_order, created_at and stops on the first rule whose match_pattern contains any pipe-separated fragment as a LITERAL SUBSTRING (position(), never a regex). A rule with no pattern is a catch-all and returns immediately; every live workspace has exactly one, at 9999. '
  '⚠ owner_de_id COMES OFF THE ROW THAT MATCHED, never off a second lookup keyed on the category — two rules may share a set_category, and resolving the owner separately would sometimes answer from a different rule than the one that produced the label. One classification, one answer, used for BOTH the routing and the stored label (the founder''s decision, 2026-08-18). '
  '⚠ It is returned ONLY for an eligible employee: same tenant, not the Workspace Assistant, lifecycle not paused/retired/archived/designed. Otherwise NULL, and the caller falls back to whatever it did before — which is what makes an owner who has left, or has not been published yet, harmless rather than a dead end. '
  '⚠ CALLERS MUST PASS THE CUSTOMER''S QUESTION AND NOTHING ELSE. The trigger historically passed `subject || '' '' || content`, and widget-ask stores the first message as "[channel · display name · account ref] question" — so an account called "Fireside Media" asking about invoice payment terms classified Safety/urgent/sev1, because `fire` is a substring and Safety sits at rule_order 10. Proven live 2026-08-18. Under routing that would be a company NAME deciding who answers. '
  'service_role ONLY (migration 233 granted authenticated; migration 662 revoked it). Returns jsonb.';

revoke all on function public.classify_support_text(uuid, text)
  from public, anon, authenticated;
grant execute on function public.classify_support_text(uuid, text)
  to service_role;


-- ===========================================================================
-- PART 4 — decide_discovery_proposal LEARNS WHO ANSWERS.
--
-- Carried forward WHOLE from migration 754, because `create or replace`
-- replaces the entire body and a function cannot be patched in place. The
-- five edits are marked `⚠ 760` in the text below; everything else is byte
-- for byte what production is running, verified by reading pg_proc rather
-- than by trusting the file (they agreed: 104474 bytes, sha b254b4b807792c59).
-- ===========================================================================
...
comment on function public.decide_discovery_proposal(uuid, text, text, uuid) is
  ...
  '⚠ THE BAND IS THE ROUTING DECISION FOR THE LABEL. classify_support_text walks the rules ORDER BY rule_order, created_at and returns on the FIRST match; the column defaults to 100 and every live tenant already carries a baseline "How-to" rule at exactly 100, so a defaulted rule ties with it and loses on a timestamp no screen shows. Above 100 means an interview topic never outranks Safety (10) or Security (20); below 9999 means it is still consulted before the catch-all. '
  '⚠⚠ AND SINCE MIGRATION 760 IT ALSO DECIDES WHO ANSWERS. The payload may carry `owner_ref` — "archetype:<key>", never an employee id, because at the moment the card is written that employee does not exist yet — and this resolves it the way 753 resolves a trust rule''s de_ref: to the employee an ACCEPTED employee proposal IN THE SAME SESSION created, re-read against this tenant, excluding the Workspace Assistant and any lifecycle that cannot front customer chat (paused/retired/archived/designed). An owner_ref that resolves to nobody REFUSES rather than degrading to an unowned rule: the card named a person, and a rule that silently does not route to them is a card that said one thing and a workspace that does another. `owner_ref` is OPTIONAL, and absent means "the workspace''s usual choice", which is byte-for-byte the behaviour of every conversation created before today. '
  '⚠ THE OWNER IS A CONSENTED LITERAL AND IS COMPARED IN BOTH DIRECTIONS. `v_ct_rule.owner_de_id is distinct from v_ct_owner` refuses a row that routes when the card promised nobody AND a row that routes nowhere when the card named somebody. The first of those is the direction nobody tests, and it is what stops a browser assigning an owner the customer never saw. '
  'The accept returns and audits systems_installed/watchers_created/watchers_skipped/guardrails_created/sop_snapshot_published for a hire, rule_type/pattern/severity/applies_to/compliance_pack_key for a guardrail, playbook_key/name/status/kind/steps_described/steps_drafted/sop_chars for a procedure, policy_id/current_level/stated_cap/stated_cap_unit/ladder_written/enforced_dials_for_this_scope/enforces_today for a trust rule, and rule_id/topic_name/set_category/match_pattern/rule_order/rules_ahead_of_it/rules_sharing_its_position/owner_de_id/owner_name/routes_to_employee/labels_conversations for a conversation topic — because a silent zero (or a silent literal) is indistinguishable from one nobody measured. `routes_to_employee` is an EXPRESSION over the resolved owner, not a literal: a hardcoded true would be the same defect the hardcoded false became. '
...
-- ===========================================================================
-- PART 5 — THE STANDING CHECK GAINS PROBE 19, AND PROBE 18'S PIN IS FLIPPED.
--
-- ⚠⚠ THIS IS THE BLOCKER 754 SHIPPED, AND IT IS THE RIGHT BLOCKER. Its probe
-- 18 asserts `routes_to_employee` is false and `labels_conversations` is
-- true, with `coalesce(..., true)` on the first so that REMOVING the key
-- fires it too. That pin exists to stop somebody making topics route without
-- moving the customer-facing copy, and it is doing its job today.
--
-- ⚠ BUT IT WOULD NOT HAVE GONE RED ON ITS OWN, and that is the more
-- important half. None of probe 18's ten payloads carries an owner_ref, so
-- after this migration they still route nothing and the pin still passes.
-- A standing check that stays green while the platform's behaviour changes
-- completely is zero findings from zero comparisons. So probe 18's arm is
-- KEPT and its message rewritten to say what it now means — "no match keeps
-- today's behaviour exactly", pinned — and PROBE 19 is added to drive the
-- other arm. Neither is sufficient alone: 18 alone goes green on a build
-- where routing was never wired, 19 alone goes green on a build where every
-- topic routes whether the customer asked for it or not.
--
-- Carried forward WHOLE from 754 for the same reason as PART 4 (372799
-- bytes, sha 537e1234b20e7f9e, file and production identical).
-- ===========================================================================
...
comment on function public.verify_decide_discovery_proposal() is
  'Behavioural probe over decide_discovery_proposal, run by certify. NINETEEN probes as of migration 760. Returns findings plus a `note:` line carrying probes_completed, probes_attempted and the assertion count, so zero comparisons can never look like a clean result. '
  'Every probe drives real calls inside a sub-block that always aborts, so nothing it writes survives; the rollback section re-counts discovery_proposals, discovery_sessions, connectors, decision audit events, digital_employees, playbook_definitions, guardrail_rules, trust_policies, de_autonomy, support_triage_rules and — since 760 — de_conversations and de_messages, scoped to the two probe tenants, and names survivors rather than only counting them. '
  'Probe 18 (754) drives the conversation_type path: TEN topics in ONE session, each created under RLS by the workspace owner and stamped here, plus classify_support_text both ways, the identity index both ways, support_triage_rules_source_proposal_uq under a same-proposal double insert, the DELETE a customer would use, and eight refusals. None of its ten payloads names an owner, so it is also the arm that pins the founder''s "no match keeps today''s behaviour exactly". '
  'Probe 19 (760) drives the other arm — A TOPIC DECIDING WHO ANSWERS. An employee is hired through the ordinary accept in the same session, a topic naming them is accepted, and then: the accept must report routes_to_employee=true with that employee''s id; classify_support_text must hand that employee back for the topic''s own words and NOBODY back for unmatched text or for a baseline rule; a RETIRED owner must fall back while the label survives; a cross-workspace owner must raise 23503; deleting the owning employee must SUCCEED and leave the rule with its tenant intact and its owner cleared (the Postgres 15 column list on ON DELETE SET NULL is what makes that true rather than a de-facto RESTRICT); three refusals must fire, including a row carrying an owner the card never named; and trg_triage_support_conversation is driven FOR REAL on three fixture conversations to show that writing only `category` leaves severity NULL — which is why the three edge writers must stamp all four columns together. '
...
-- PART 6 — THE MIGRATION VERIFIES ITSELF, AND ROLLS BACK IF IT CANNOT.
--
-- 754 shipped WITHOUT one of these and would have applied silently no matter
-- what its probes said; 751's caught its own defect at apply time and rolled
-- back with the ledger untouched. This block asserts the schema this file
-- creates, proves the classifier rewrite changed no existing answer, ratchets
-- the router's new branch, and then runs the standing check and refuses a red.
-- ===========================================================================
do $$
declare
...
begin
  ------------------------------------------------------------------------
  -- (1) THE SCHEMA THIS FILE CREATES.
  ------------------------------------------------------------------------
  v_checks := v_checks + 1;
  if not exists (
    select 1 from pg_constraint c
     where c.conrelid = 'public.digital_employees'::regclass
       and c.conname = 'digital_employees_tenant_id_id_uq'
       and c.contype = 'u') then
    v_bad := array_append(v_bad, '760: digital_employees_tenant_id_id_uq is missing. It is the reference target support_triage_rules_owner_de_fk points at, and without it the composite foreign key cannot exist — which means "who answers" would be an id with a tenant filter somebody could delete, instead of a boundary the database holds.');
  end if;

  v_checks := v_checks + 1;
  if not exists (
    select 1 from information_schema.columns
     where table_schema = 'public' and table_name = 'support_triage_rules'
       and column_name = 'owner_de_id') then
    v_bad := array_append(v_bad, '760: support_triage_rules.owner_de_id is missing. classify_support_text reads it to answer WHO ANSWERS, and every routing arm in probe 19 is a statement about it.');
  end if;

  -- ⚠ THE CONSTRAINT IS READ, NOT ASSUMED, AND ITS ACTION IS READ WITH IT.
  -- `confdeltype` distinguishes n(SET NULL) from c(CASCADE) and a(NO ACTION),
  -- and `confdelsetcols` is the Postgres 15 column list — an EMPTY one means
  -- SET NULL would try to null tenant_id, which is NOT NULL, so the whole
  -- constraint would behave as RESTRICT. That is the trap this arm exists for,
  -- and probe 19(g) drives it as well as reading it.
  v_checks := v_checks + 1;
  select c.confdeltype::text || ':' || coalesce(array_length(c.confdelsetcols, 1)::text, '0')
    into v_confdef
    from pg_constraint c
   where c.conrelid = 'public.support_triage_rules'::regclass
     and c.conname = 'support_triage_rules_owner_de_fk';
  if v_confdef is distinct from 'n:1' then
    v_bad := array_append(v_bad, format('760: support_triage_rules_owner_de_fk reads %L, expected "n:1" (ON DELETE SET NULL with exactly ONE column in the Postgres 15 column list). "c:*" is CASCADE, which would DELETE A CUSTOMER''S TOPIC because an employee left. "a:*" is NO ACTION, which would make the employee undeletable. "n:0" is the subtle one — SET NULL with no column list nulls tenant_id too, tenant_id is NOT NULL, and the delete raises 23502: a de-facto RESTRICT wearing the right keyword.', coalesce(v_confdef, 'ABSENT')));
  end if;

  -- ...and that it is COMPOSITE. A single-column FK would pass every arm above
  -- and admit an owner from another workspace.
  v_checks := v_checks + 1;
  select count(*) into v_n
   ...
   where c.conrelid = 'public.support_triage_rules'::regclass
     ...
     and c.confrelid = 'public.digital_employees'::regclass;
  if coalesce(v_n, 0) <> 1 then
    v_bad := array_append(v_bad, '760: support_triage_rules_owner_de_fk is not a TWO-COLUMN foreign key into digital_employees. A plain reference to the id alone admits an owner from another workspace — the write succeeds, the lookup''s tenant filter then makes the topic silently stop routing, and nothing says so.');
  end if;

  ------------------------------------------------------------------------
  -- (2) THE CLASSIFIER REWRITE CHANGED NO EXISTING ANSWER.
  --
  -- ⚠ THIS IS A BEFORE/AFTER COMPARISON OVER LIVE DATA, not a spot check. The
  -- temp table was filled while the OLD function was still installed; this
  -- re-runs the NEW one over the same corpus and compares the four keys that
  -- existed before, with rule_id and owner_de_id stripped. A vacuity guard
  -- comes first: an empty corpus would make "zero differences" meaningless,
  -- and 177 rows is what this platform actually has.
  ------------------------------------------------------------------------
  v_checks := v_checks + 1;
  select count(*) into v_corpus from _760_cls_before;
  if coalesce(v_corpus, 0) < 100 then
    v_bad := array_append(v_bad, format('760: the classifier before/after corpus holds %s row(s). It is every first user message on a triaged channel and read 177 when this migration was written; below 100 the comparison below is not comparing enough to mean anything, and zero differences from zero rows looks exactly like a clean result.', coalesce(v_corpus, 0)));
  end if;

  v_checks := v_checks + 1;
  select count(*),
         max(b.conv_id::text)
    into v_moved, v_example
    from _760_cls_before b
    join lateral (
      select coalesce(c.subject, '') || ' ' || coalesce(m.content, '') as txt
        from public.de_conversations c
        join lateral (
          select mm.content from public.de_messages mm
           where mm.conversation_id = c.id and mm.role = 'user'
           order by mm.created_at, mm.id limit 1
        ) m on true
       where c.id = b.conv_id
    ) t on true
   where (public.classify_support_text(b.tenant_id, t.txt) - 'rule_id' - 'owner_de_id')
         is distinct from b.cls;
  if coalesce(v_moved, 0) <> 0 then
    v_bad := array_append(v_bad, format('760: the rewritten classify_support_text returns a DIFFERENT answer for %s of %s live first messages (one of them is conversation %s). The two new keys are additive by design and the four old ones must be byte-identical — every existing reader (the trigger, the inbox filter, the History report) selects on those four, and a moved answer would re-file real customer traffic as a side effect of adding an owner column.', v_moved, v_corpus, coalesce(v_example, 'unknown')));
  end if;

  -- ...and THE NO-RULE DEFAULT PATH, WHICH THE CORPUS CANNOT REACH. This arm
  -- was added after an inversion round found the hole: changing the default
  -- answer from 'general' to something else left the 177-row comparison above
  -- completely green, because all 18 live workspaces carry a pattern-less
  -- catch-all at rule_order 9999 and `classify_support_text` returns on it
  -- before it can ever fall through. So the ONE path no live tenant exercises
  -- is pinned directly, against a tenant id that owns no rules at all — which
  -- is also the state a workspace is in between being created and being seeded
  -- (migration 742 found three workspaces sitting in exactly that state,
  -- silently labelling everything 'general').
  v_checks := v_checks + 1;
  declare
    v_dflt jsonb := public.classify_support_text('00000000-0000-0000-0000-000000000000'::uuid, 'anything at all');
  begin
    if (v_dflt ? 'owner_de_id') is not true or (v_dflt ? 'rule_id') is not true
       or coalesce(v_dflt ->> 'category', '') <> 'general'
       or coalesce(v_dflt ->> 'priority', '') <> 'normal'
       or coalesce(v_dflt ->> 'severity', '') <> 'sev3'
       or coalesce(v_dflt ->> 'rule', '') <> 'default'
       or (v_dflt ->> 'rule_id') is not null
       or (v_dflt ->> 'owner_de_id') is not null then
      v_bad := array_append(v_bad, format('760: classify_support_text''s no-rule default returned %L. It has to be the migration 233 answer exactly — general / normal / sev3 / default — plus a NULL rule_id and a NULL owner_de_id. The three conversation writers read `owner_de_id` off EVERY call, so an absent key reads as "nobody" and a non-null one on this path would route a workspace with no rules at all. And the 177-message comparison above cannot see this: every live workspace has a pattern-less catch-all at 9999, so nothing ever falls through to here.', left(coalesce(v_dflt::text, 'NULL'), 300)));
    end if;
  end;

  -- ⚠ AND THE INVERSION THE FOUNDER'S DECISION 1 RESTS ON, DRIVEN HERE RATHER
  -- THAN QUOTED. The end-user tag has to be able to change the answer, or the
  -- "0 of 177 differ" measurement in this file's header would be zero findings
  -- from zero comparisons. It is driven against a REAL tenant's real rules.
  v_checks := v_checks + 1;
  select count(*) into v_n
    from public.tenants t
   where exists (select 1 from public.support_triage_rules r where r.tenant_id = t.id and r.active)
     and public.classify_support_text(t.id, 'what are the payment terms on my invoice?') ->> 'category'
         is distinct from
         public.classify_support_text(t.id, '[widget · Fireside Media · account FIRE-1] what are the payment terms on my invoice?') ->> 'category';
  if coalesce(v_n, 0) = 0 then
    v_bad := array_append(v_bad, '760: NO live workspace classifies "invoice payment terms" differently once an end-user display name of "Fireside Media" is prefixed. That prefix is what widget-ask actually stores, `fire` is a substring the Safety rule matches, and Safety sits at rule_order 10 ahead of Billing at 60 — it read TRUE for every seeded workspace on 2026-08-18. A zero here means either the baseline rules have changed or this comparison has stopped comparing, and the header''s "0 of 177 differ" would then be a statement about nothing.');
  end if;

  ------------------------------------------------------------------------
  -- (3) THE RATCHET ON THE ROUTER'S NEW BRANCH.
  --
  -- Same construction 754 uses and for the same reasons: comments stripped
  -- (this function discusses everything it does at length in prose), read with
  -- `position` rather than `like` (`_` is LIKE's wildcard and every identifier
  -- here has one), a vacuity guard first, and every pin counted for uniqueness
  -- afterwards — a pin that occurs twice is a coincidence, not a pin.
  ------------------------------------------------------------------------
  v_checks := v_checks + 1;
  declare
    v_src     text;
    ...
    v_pin     text;
    v_pins    text[] := array[
      'v_ct_own_ref := nullif(btrim(v_p.payload ->> ''owner_ref''',
      'v_ct_own_ref !~ ''^archetype:[a-z0-9_]+$''',
      'v_ct_own_arch := substring(v_ct_own_ref from 11)',
      'select s.created_object_id into v_ct_owner',
      'select d.name into v_ct_own_name',
      -- ⚠ A COMPOUND PIN, and it is compound because the obvious short ones
      -- are NOT UNIQUE and the ratchet said so on its first run: `and s.kind =
      -- ''employee''` occurs twice (753''s trust_rule branch resolves its
      -- de_ref the same way) and `coalesce(d.is_workforce_assistant, false) =
      -- false` occurs three times. A pin that matches somewhere else is
      -- satisfied by code that has nothing to do with what it guards. Anchoring
      -- it on `v_ct_owner`, which exists only in this branch, makes it unique.
      'where d.id = v_ct_owner
             and d.tenant_id = v_p.tenant_id
             and coalesce(d.is_workforce_assistant, false) = false',
      'v_ct_rule.owner_de_id is distinct from v_ct_owner',
      '''routes_to_employee'', (v_ct_owner is not null)',
      '''owner_de_id'',        v_ct_owner'
    ];
  begin
    ...
    if v_src is null or length(v_src) < 6000 or position('case v_p.kind' in v_src) = 0 then
      v_bad := array_append(v_bad, format('760: the comment-stripped body of decide_discovery_proposal came back %s characters long and %s the `case v_p.kind` dispatch. The ratchet cannot tell a missing branch from an unreadable body, so it is refusing to answer rather than guessing.',
        coalesce(length(v_src)::text, 'NULL'),
        case when position('case v_p.kind' in coalesce(v_src, '')) > 0 then 'contains' else 'does NOT contain' end));
    else
      foreach v_pin in array v_pins loop
        if position(v_pin in v_src) = 0 then
          v_missing := array_append(v_missing, format('%L', v_pin));
        elsif (length(v_src) - length(replace(v_src, v_pin, ''))) / greatest(length(v_pin), 1) <> 1 then
          v_dupes := array_append(v_dupes, format('%L occurs %s time(s)', v_pin,
            (length(v_src) - length(replace(v_src, v_pin, ''))) / greatest(length(v_pin), 1)));
        end if;
      end loop;

      -- ⚠⚠ THE NEGATIVE PINS, CARRIED FORWARD FROM 754 AND EXTENDED. Everything
      -- this file says about Path B is a promise that this function does NOT
      -- write the rule itself. A positive pin can be satisfied by code that
      -- also does something else; only these say the something else is absent.
      -- The de_conversations pair matters MORE now, not less: 754 forbade
      -- touching that table because "accepting a topic creates a RULE for
      -- conversations that have not happened yet". That is still true, and
      -- now that a topic really does decide who answers, a write here would be
      -- the accept quietly re-pointing threads a customer never agreed to move
      -- — with de_id, the attribution column, sitting right there.
      if position('insert into public.support_triage_rules' in v_src) > 0
        ...
        v_missing := array_append(v_missing, '⚠ NEGATIVE PIN FIRED: decide_discovery_proposal now writes public.support_triage_rules directly. This RPC is SECURITY DEFINER and runs as postgres, so a write here bypasses support_triage_rules_admin_write entirely and is the second creation engine contract §8.3 forbids');
      end if;
      if position('update public.de_conversations' in v_src) > 0
         or position('insert into public.de_conversations' in v_src) > 0 then
        v_missing := array_append(v_missing, '⚠ NEGATIVE PIN FIRED: decide_discovery_proposal now writes public.de_conversations. Accepting a topic writes a RULE for conversations that have not happened yet; touching that table would re-point threads the customer never agreed to move, and de_id — which every employee-performance reader counts by — lives there');
      end if;

      if coalesce(array_length(v_missing, 1), 0) > 0 then
        v_bad := array_append(v_bad, format('760: decide_discovery_proposal''s CODE (comments stripped, %s characters) is missing %s of the owner branch''s load-bearing parts, or gained one it must not have: %s. Probe 19 would be aborting or answering about something else rather than failing.',
          length(v_src), array_length(v_missing, 1), array_to_string(v_missing, '; ')));
      end if;
      if coalesce(array_length(v_dupes, 1), 0) > 0 then
        v_bad := array_append(v_bad, format('760: %s ratchet pin(s) do not occur EXACTLY ONCE in the comment-stripped body: %s. A pin that also matches somewhere else is satisfied by code that has nothing to do with what it guards — which is how 751''s `compliance_pack_key is not null` stayed green while the protection behind it could have been deleted.',
          array_length(v_dupes, 1), array_to_string(v_dupes, '; ')));
      end if;
    end if;
  end;

  -- ...and the same discipline applied to the CLASSIFIER, because the owner
  -- lookup living there is the whole "one classification, one answer" decision.
  v_checks := v_checks + 1;
  declare
    v_csrc text;
  begin
    select regexp_replace(p.prosrc, '--[^' || chr(10) || ']*', '', 'g') into v_csrc
      from pg_proc p join pg_namespace n on n.oid = p.pronamespace
     where n.nspname = 'public' and p.proname = 'classify_support_text';
    if v_csrc is null or length(v_csrc) < 600 then
      v_bad := array_append(v_bad, format('760: classify_support_text''s comment-stripped body came back %s characters. The three conversation writers read their whole answer from it; an unreadable body means the pins below compared nothing.', coalesce(length(v_csrc)::text, 'NULL')));
    else
      if position('from digital_employees d' in v_csrc) = 0
         or position('coalesce(d.is_workforce_assistant, false) = false' in v_csrc) = 0
         or position('not in (''paused'', ''retired'', ''archived'', ''designed'')' in v_csrc) = 0
         or position('d.tenant_id = p_tenant_id' in v_csrc) = 0 then
        v_bad := array_append(v_bad, '760: classify_support_text no longer resolves the owner with all four of its filters (the digital_employees read, the Workspace Assistant exclusion, the four ineligible lifecycles, and the same-tenant clause). ⚠ THE WORKSPACE-ASSISTANT EXCLUSION IS PINNED HERE AND NOWHERE ELSE: probe 19 cannot drive it, because driving it would mean creating a workspace assistant, and that is the one row this workstream may not touch. A pin is what it gets instead, and it is stated as such rather than counted as proof.');
      end if;
      if position('exit rules' in v_csrc) = 0 then
        v_bad := array_append(v_bad, '760: classify_support_text no longer carries the single-exit loop. It was restructured to ONE return precisely so the owner lookup exists once — two returns would mean the same expression written twice, and two copies of a lookup is how the catch-all path and the match path come to disagree.');
      end if;
    end if;
  end;

  ------------------------------------------------------------------------
  -- (4) NOTHING IN PRODUCTION GAINED AN OWNER BY ACCIDENT.
  --
  -- The column is new and nullable, so every one of the rules that existed
  -- before this file must still route nothing. That is the founder's decision
  -- 3 asserted against the actual table rather than against a comment.
  ------------------------------------------------------------------------
  v_checks := v_checks + 1;
  select count(*), count(*) filter (where owner_de_id is not null)
    into v_rules_n, v_owned_n
    from public.support_triage_rules;
  if coalesce(v_owned_n, 0) <> 0 then
    v_bad := array_append(v_bad, format('760: %s of %s triage rules already carry an owner immediately after the column was added. "Purely additive" means every workspace behaves exactly as it did until somebody decides otherwise, and a backfill — however well meant — is that decision being made for them.', v_owned_n, v_rules_n));
  end if;

  ------------------------------------------------------------------------
  -- (5) THE STANDING CHECK, RUN FOR REAL.
  ------------------------------------------------------------------------
  ...
  -- ⚠ EQUALITY, NOT A FLOOR. 20 means somebody added a probe without moving
  -- probes_attempted or certify's EXPECTED_PROBES; 18 means one stopped running.
  v_checks := v_checks + 1;
  if coalesce(v_probes, 0) <> 19 then
    v_bad := array_append(v_bad, format('the function reported probes_completed=%L, expected 19. Zero findings from zero probes looks exactly like a clean result, which is the whole reason this number is printed. ⚠ If probe 19 is the missing one, read its abort line above before moving this number: it is the only place that drives a topic actually choosing an employee, and lowering the denominator would leave probe 18 pinning "nothing routes" as the only statement about routing this platform makes.', coalesce(v_probes::text, 'ABSENT')));
  end if;

  -- ⚠ NOT a pinned constant — pinning it would go red every time a probe
  -- legitimately gains a check, and somebody would then edit the pin rather
  -- than read it. What must never happen is the number COLLAPSING.
  v_checks := v_checks + 1;
  if coalesce(v_asserts, 0) < 287 then
    v_bad := array_append(v_bad, format('the function compared only %L assertion(s). 741 carried 95, 745 carried 98, 746 carried 138, 751 carried 169, 752 carried 202, 753 carried 239, 754 carried 265, and this carries 287. COUNTED, NEVER ESTIMATED: the number is the count of `v_checks := v_checks + 1` in the SHIPPED verifier body — the span from `CREATE OR REPLACE FUNCTION public.verify_decide_discovery_proposal()` to its `$function$` and nothing else — measured by the script that assembled this migration, which read 265 for 754''s body and 287 for this one. The delta is PROBE 19''s own arms plus ONE rollback arm (de_conversations and de_messages, asserted together). ⚠ A RAW GREP OF THIS FILE IS NOT THIS NUMBER: it also counts this DO block''s own, and every occurrence sitting in prose including the one in this sentence. A collapse means probes are being skipped rather than run. scripts/certify.mjs carries the same floor and must move with it.', coalesce(v_asserts::text, 'ABSENT')));
  end if;
  ...
  ------------------------------------------------------------------------
  -- (6) PERIMETER on the verifier itself, and on the classifier.
  ------------------------------------------------------------------------
  v_checks := v_checks + 1;
  if has_function_privilege('anon', v_sig, 'execute')
     or has_function_privilege('authenticated', v_sig, 'execute')
     or has_function_privilege('public', v_sig, 'execute') then
    v_bad := array_append(v_bad, 'anon, authenticated or PUBLIC can execute verify_decide_discovery_proposal — it impersonates real user ids through request.jwt.claim.sub');
  end if;
  v_checks := v_checks + 1;
  ...
  -- ⚠ AND THE CLASSIFIER'S OWN PERIMETER, WHICH THIS FILE WIDENS THE ANSWER OF.
  -- It now returns a digital employee's id. Migration 233 granted it to
  -- `authenticated` and migration 662 revoked that; if the grant ever comes
  -- back, any signed-in browser could read another workspace's rule names,
  -- categories AND now the employee behind each one, by passing a tenant id.
  v_checks := v_checks + 1;
  if has_function_privilege('anon', 'public.classify_support_text(uuid, text)', 'execute')
     or has_function_privilege('authenticated', 'public.classify_support_text(uuid, text)', 'execute')
     or has_function_privilege('public', 'public.classify_support_text(uuid, text)', 'execute') then
    v_bad := array_append(v_bad, 'anon, authenticated or PUBLIC can execute classify_support_text. It takes a tenant id as a PARAMETER and now returns an employee id with the classification, so a grant here is migration 662''s "a tenant-id parameter is not a password" with a person''s identity attached. service_role only.');
  end if;
  v_checks := v_checks + 1;
  if not has_function_privilege('service_role', 'public.classify_support_text(uuid, text)', 'execute') then
    v_bad := array_append(v_bad, 'service_role CANNOT execute classify_support_text — widget-ask, email-inbound, de-answer and the triage trigger all read their answer from it, so nothing would be labelled or routed at all');
  end if;

  ------------------------------------------------------------------------
  -- THE ASSERTION FLOOR ON THIS BLOCK ITSELF. A block that compared nothing
  -- and found nothing reads exactly like a clean apply.
  ------------------------------------------------------------------------
  if v_checks < 19 then
    raise exception '760: this verification block made only % comparison(s), and it has 19 arms. It is refusing to report a clean apply on a run where most of it did not execute.', v_checks;
  end if;

  if coalesce(array_length(v_bad, 1), 0) > 0 then
    raise exception '760: % of % assertion(s) failed: %',
      array_length(v_bad, 1), v_checks, array_to_string(v_bad, ' | ');
  end if;

  raise notice '760: a topic decides who answers — verify_decide_discovery_proposal() returned CLEAN on its first run after the owner branch landed: % probe(s) completed, % assertion(s) compared, 0 findings, and % assertion(s) about this migration itself passed. classify_support_text now returns the matched rule and its ELIGIBLE owner from ONE pass, and returned a byte-identical four-key answer for all % live first messages it was re-run over. % triage rule(s) exist and % carry an owner, which is the founder''s "no match keeps today''s behaviour exactly" measured rather than promised. %',
    v_probes, v_asserts, v_checks, v_corpus, v_rules_n, coalesce(v_owned_n, 0), coalesce(v_notes[1], '(no note)');
end $$;
