...
// mig 823's ratchet, and the sibling of the trigger arm above: the EXECUTE
// allowlist NAMED de_kpi_action_value correctly and could still have been
// cleared by one --pin-allowlist run, because its finding line is the same
// shape for a cross-tenant SECDEF hole as for a benign new grant. This one has
// no allowlist and nowhere to add one.
import { tenantParamExecutePerimeterSql } from './tenant-param-execute-perimeter.mjs';
// Register item A-3's compensating control. The GRANT half of the outbound-HTTP
// perimeter; netExposureFailures() below is the EXPOSURE half, and neither is
// sufficient alone. A-3 stays OPEN and BLOCKED because no revoke available to
// `postgres` moves it — proven, and the proof is why this file exists rather
// than a migration: the revokes RUN CLEAN AND CHANGE NOTHING.
import { netOutboundPerimeterSql } from './net-outbound-perimeter.mjs';
// Register item A-1, the same shape one schema over and with a THIRD owner
// role. Its default-ACL half WAS fixable and is fixed (mig 839); its live-grant
// half is blocked the same way A-3 is, so the arm reports it as KNOWN-BLOCKED
// on every green run and fails the day it clears. It also fails on LOSS, which
// A-3's does not need to and this one does: storage.objects is the upload path.
import { storageWritePerimeterSql } from './storage-write-perimeter.mjs';
...
const PROBES = [
  ...
  },
  {
    ...
    why: 'mig 710 built the Gap-2 seam: repeated identical landed human approvals become ONE trust_promotion proposal on the existing queue. Two proofs, held by privilege and by the ledger, never by promise. PRIVILEGE: the writer runs as the NOLOGIN role trust_pattern_proposer, which holds no EXECUTE on decide_human_task/apply_trust_promotion/trust_apply_level/trust_demote/set_de_autonomy, no UPDATE/DELETE on human_tasks, no write on de_autonomy (the dial) or approval_authority (the limits), and UPDATE on trust_policies only for the four request-bookkeeping columns — plus a reachable-decider sweep (the two-paths trap) and a cannot-file liveness arm (a proposer that silently lost INSERT is the mig-625 built-but-unfed breaker with a privilege cause). ⚠ AND THE SEAM MUST BE FED, PER WRITER: the daily governance sweep is the only heartbeat (cron de-governance-sweep-daily is one statement, select de_governance_sweep_internal()), so a writer its body does not name never runs. That arm watched ONE writer while mig 828 shipped a SECOND — request_eligible_promotions — with no caller at all, and stayed green for it: the built-and-starved defect committed by the control named after it. Mig 834 wires the call and the arm is now per-writer, with the new writer\'s row gated on its callee EXISTING so an environment behind 828 is not red for a defect it does not have. Proven across five states against production in aborting transactions: 828 absent -> silent; 828 applied and 834 not -> RED naming the new writer (the exact state nothing reported); both applied -> silent; each call stubbed out of the installed sweep in turn -> RED naming that writer. EVIDENCE: every OPEN system-raised proposal is re-derived from the ledger at probe time, never trusted from its stored payload (mig 642) — a PATTERN-FILED proposal (the detector, raise_trust_widening_proposals) must cite >= 3 decisions the ledger RE-CONFIRMS as approved + production-origin + landed; a CRITERIA-FILED proposal (mig 828\'s second writer, request_eligible_promotions) never claimed a citation to re-verify, so its policy\'s OWN criteria are re-asked instead (trust_evidence_for). The two populations must always sum to the total scanned — checked on every run, not assumed. Suspended workspaces must hold none; a pending proposal no policy points at is unactionable and flagged. The denominator prints on every run; zero open proposals is legal and says so',
  ...
  {
    name: 'net-outbound-primitive-not-widened',
    why: 'Register item A-3, and the trap is recorded IN the item: `net.http_post/get/delete` are EXECUTE-able by anon and authenticated, and a REVOKE running as postgres is a silent no-op because schema `net` is owned by supabase_admin. ⚠⚠ THAT WAS DRIVEN, NOT INFERRED, ON PRODUCTION 2026-08-21 IN A ROLLED-BACK TRANSACTION, AND IT IS WORSE THAN "no-op": seven top-level REVOKEs — from anon, from authenticated, from PUBLIC, on the three routines, on both net tables, and `revoke usage on schema net` — ALL RAN WITH NO ERROR, and anon still held EXECUTE, still held schema USAGE, and still held INSERT on net.http_request_queue afterwards. PostgreSQL does not RAISE when the revoker lacks grant option; it warns, and the Management API discards warnings. The one visible effect is the disguise: proacl moves from NULL to `{=X/supabase_admin,supabase_admin=X/supabase_admin}` — the ACL is MATERIALISED and nothing is removed, so a checker asserting "proacl is no longer NULL" would go green off a change that revoked nothing. Only `alter function ... owner to postgres`, `set role supabase_admin` and `alter default privileges for role supabase_admin` raise 42501 (the same wall mig 715 hit). NO MIGRATION WAS SHIPPED, because one would have applied cleanly, entered the ledger, and closed A-3 having done nothing. ⚠ SEVERITY STATED HONESTLY: LATENT, NOT LIVE. `net` is not in PostgREST\'s exposed-schema list — proven from outside with the publishable anon key, where both a table read (Accept-Profile: net) and an RPC call (Content-Profile: net) return HTTP 406 PGRST106 "Only the following schemas are exposed: public, graphql_public", against a control request that correctly 404s inside `public`. And there is no in-database bridge: all 24 net callers are SECURITY DEFINER owned by postgres, the four reachable by `authenticated` take ZERO arguments and hard-code their URL, and the three "SECURITY INVOKER and reachable" hits are false positives (an event_trigger function that cannot be invoked, plus `net.uk`/`ukr.net` string literals in the email-domain denylist). ⚠ THE SURFACE IS WIDER THAN A-3 RECORDED, which is why the baseline is 16 and not 3: net.http_request_queue and net._http_response carry an EXPLICIT PUBLIC grant of arwdDxtm, so the day `net` is exposed it is not "call http_post" but "insert into the worker queue and read every response body" — and queue rows carry the Authorization header of every internal dispatch. ⚠⚠ SO WHAT THIS ARM RATCHETS IS EVERYTHING AROUND THE UNFIXABLE BASELINE, all of which CAN fail: a net grant whose GRANTOR IS NOT supabase_admin (that one is ours and IS revocable — 0 today); the surface WIDENING past the 16 recorded objects; the surface NARROWING, which fails too, loudly, because good news means A-3 has expired and must be re-measured rather than left open forever; a function outside `net` reachable by anon or PUBLIC whose body references a real net object (absolute — an anonymous caller has no identity, so there is no legitimate instance); and the SSRF shape, a reachable net-referencer taking a caller-supplied text/json parameter. The baseline is a CONSTANT IN THE SOURCE FILE, not a JSON allowlist, so no --pin-allowlist / --pin-write / --pin-edge run can regenerate it — widening it costs a commit a human reads. FIVE VACUITY ARMS: zero net objects, has_function_privilege having stopped discriminating, an empty name sieve (which would clear every function that has ever existed), zero net callers found among 1109 functions, and a comment strip that has stopped stripping — the last one is not hypothetical, decide_discovery_proposal is authenticated-reachable, takes text parameters, and NAMES net.http_post in a comment, so without the strip it reports as an SSRF bridge every run. Denominator prints on a PASS. Mutation-tested both directions INCLUDING THE ONE EXEMPTION INVERTED: `node scripts/net-outbound-perimeter.mjs --selftest` — 25 mutations, each asserting a COUNT DELTA, with the trigger-shaped control re-run byte-identical except trigger_shaped=false to prove the exclusion is the trigger shape and not a dead arm',
    sql: netOutboundPerimeterSql(),
  },
  {
    name: 'storage-write-perimeter',
    why: 'Register item A-1 — the file store, which had NO gate at all. ⚠ THE ITEM NAMED TWO THINGS AND THEY HAVE DIFFERENT ANSWERS; both were driven on production 2026-08-21 in rolled-back transactions rather than reasoned about. (a) BLOCKED: storage.objects and storage.buckets grant anon AND authenticated arwdDxtm, TRUNCATE included — and RLS does not police TRUNCATE, so either role is one statement from emptying every tenant\'s file index. `revoke truncate, delete, insert, update on storage.objects from anon, authenticated` RETURNED SUCCESS and afterwards has_table_privilege(\'anon\',\'storage.objects\',\'TRUNCATE\') was STILL TRUE with relacl BYTE-IDENTICAL. ⚠⚠ AND THIS IS NOT THE pg_net SHAPE: there `postgres` held no grant option at all, here it holds a*r*w*d*D*x*t*m* WITH grant option — select_best_grantor picks postgres as the grantor, finds no aclitem with grantee=anon and grantor=postgres, and removes nothing. Holding grant option is not being the grantor, and a checker built on "does postgres have grant option" would have called this fixable and shipped a migration that closed the item having done nothing. The walls that DO speak: `alter table storage.objects owner to postgres` -> 42501 must be owner; `set role supabase_storage_admin` -> 42501. Escalation written up for a role that can run it: docs/72-supabase-privilege-escalations.md. (b) FIXED by mig 839: schema `storage` has NO supabase_admin default-ACL row — unlike `public`, where the arwdDxtm row mig 715 could not move is exactly that (register item A-2, re-proven today, still 42501). All three storage rows carry grantor postgres, and a role may always alter its own default privileges, so all three now grant anon/authenticated/PUBLIC nothing. ⚠ SEVERITY OF (b) STATED HONESTLY: LATENT, NOT LIVE — `create table storage._probe (id int)` as postgres returns 42501 permission denied for schema storage (postgres holds USAGE there, never CREATE), so that row had never fired and could not. It was a loaded default, not an open door, and it is shipped because the day anything grants postgres CREATE on that schema every object born there arrives TRUNCATE-able by the internet with nothing saying so. ⚠⚠⚠ THIS ARM FAILS ON LOSS AS HARD AS ON GAIN, and that is the arm most likely to fire, because docs/72 asks a human at Supabase to run a REVOKE by hand: `authenticated` MUST KEEP SELECT/INSERT/UPDATE/DELETE on storage.objects — that is not a gap in the escalation, it IS the upload path, since the storage service reaches storage.objects as the caller\'s own role and RLS is what scopes it to the tenant. Mig 643 nearly left eleven of twelve workspaces administrable by nobody; a storage revoke one letter too far reads as hardening and deletes the product\'s ability to store a file. SIX ARMS, all fail-able: a storage grant whose grantor is not the object\'s OWN OWNER (ours, and revocable — 0 today; ⚠ the comparison is per-object because schema `storage` is owned by supabase_admin while everything inside it is owned by supabase_storage_admin, and the first draft, keyed on one constant, immediately accused upstream of a grant nobody here can revoke); the surface WIDENING past the 25 recorded objects, at LETTER granularity so a baseline object gaining one privilege is caught; the surface NARROWING, which fails too, loudly, because that means Supabase acted on docs/72 and A-1 must be re-measured and closed rather than left open on a finding that has quietly expired — that arm is how the blocked half stops being permanent; mig 839\'s default-ACL ratchet, computed through acldefault() so a MISSING pg_default_acl row is judged on what it would really mean (for FUNCTIONS the built-in default is EXECUTE TO PUBLIC, so a deleted row is a hole, not a clean slate); 29 role/privilege pairs asserted STILL HELD; and bucket privacy, deliberately asymmetric — a recorded-private bucket turning public or vanishing fails, a NEW PUBLIC bucket fails, a new PRIVATE bucket is counted and does not, because onboarding-attachments is a live design and a gate that reds on planned work gets ignored. FIVE VACUITY ARMS: zero storage objects, has_*_privilege having stopped discriminating, a short default-ACL population (arm 4 judges whatever it is handed), zero retention pairs, zero buckets. The denominator prints on a PASS and CARRIES THE KNOWN-BLOCKED ACKNOWLEDGEMENT with the docs/72 path, so the blocked half is never silently passed. Every baseline is a CONSTANT IN THE SOURCE FILE — no --pin-allowlist / --pin-write / --pin-edge run can regenerate it. Mutation-tested both directions INCLUDING THE ONE EXEMPTION INVERTED: `node scripts/storage-write-perimeter.mjs --selftest` — each mutation asserts a COUNT DELTA as well as a message, and the second-upstream-owner control is re-run byte-identical except grantor=postgres to prove the exemption is grantor==owner and not a dead arm',
    sql: storageWritePerimeterSql(),
  },
  {
    name: 'secdef-tenant-param-unreachable-by-anon',
    why: 'mig 823. de_kpi_action_value(p_tenant_id uuid, ...) was SECURITY DEFINER with proacl `{=X/postgres, ...}` — that leading `=X/` is PUBLIC, so anon and authenticated held EXECUTE by INHERITANCE rather than by a named grant, which matters because `revoke ... from anon` would have changed the ACL not at all while reading like a repair. PROVEN before it was fixed, not inferred: three POSTs to /rest/v1/rpc/de_kpi_action_value carrying only the PUBLISHABLE anon key returned outsourcetel-hq n=13, Review Lab n=53/auto 11.3%, a third tenant n=20 — every figure matching service-role ground truth, from outside every workspace, with RLS bypassed. ⚠ SEVERITY STATED HONESTLY: it returns two AGGREGATES, never a row and never a name, it writes nothing, and it needs BOTH a tenant uuid and a de uuid — so it is a cross-tenant aggregate ORACLE (how much work an employee did, how much ran unsupervised, and via source_config which CATEGORIES of work), not a dump. Real, moderate. ⚠⚠ THE POINT OF THIS ARM IS NOT THAT NOTHING SAW IT — TWO THINGS DID. execute-perimeter named it and so did secdef-caller-tenant-ratchet, both red in the 14:13 run. What neither did is say what it WAS: the allowlist line reads "NEW/CHANGED grant not in allowlist: de_kpi_action_value(...)|anon=true|authed=true", the same shape of line as the nine benign authenticated-only additions sitting beside it, with nothing in it saying SECDEF, tenant parameter, or the internet — and one --pin-allowlist run meant to record those nine would have blessed the tenth. That is the 48-of-49 laundering path the trigger arm exists for, one class over. So this arm has NO allowlist and NOWHERE TO ADD ONE. ⚠⚠⚠ IT CAN BE ABSOLUTE ONLY BECAUSE IT IS SCOPED TO anon/PUBLIC: an anonymous caller has no identity to derive a tenant from — auth.uid() is null, there is no membership row — so the parameter IS the authorisation in EVERY instance, by construction, and a rule with no legitimate counter-example needs no exemptions. ⚠ THE authenticated HALF IS DELIBERATELY NOT JUDGED HERE, and the reason is a measurement rather than an omission: 114 of the 257 tenant-param SECDEF functions are authenticated-reachable and nearly all are legitimate, so that arm needs a "does it derive?" predicate — and BOTH candidates were built and measured wrong in opposite directions. A direct-name body regex returned 7 non-derivers of which FIVE were false positives (is_ancestor_of/tenant_ancestors/tenant_descendants derive via caller_has_tenant_relationship, list_org_tree via assert_own_tenant, scim_tokens_list via can_admin_tenant_internal — helper names no fixed vocabulary knows); a transitive closure went the other way and swallowed 498 of 873 functions, clearing 57% of the catalogue by construction. Shipping a checker measured to be wrong manufactures findings AND grants cover, so that half stays with secdef-caller-tenant-ratchet — whose real weakness is named rather than papered over: its exemptions key on proname ALONE with no signature, so a new OVERLOAD of an exempted name inherits the clearance silently. This arm knows no names at all, so the anon half of that hole is closed absolutely. ⚠ ARM 2 IS SEPARATE FROM ARM 1 ON PURPOSE — an anon-only revoke is THEATRE (security_anon_guard_hole), and a NULL proacl MEANS PUBLIC=X, so a function created and never REVOKEd is born on this perimeter; acldefault() spells that out rather than letting a born-public function read as ungranted. THREE VACUITY ARMS, because zero findings from zero comparisons looks exactly like a clean result: the population must be non-zero, the tenant-parameter matcher must match SOMETHING (an empty sieve passes every function that has ever existed), and at least one function in public must still come back anon-executable or has_function_privilege has stopped discriminating. Denominator prints on a PASS as well as a fail. Mutation-tested both directions: `node scripts/tenant-param-execute-perimeter.mjs --selftest` — 11 mutations, each asserting a COUNT DELTA as well as a message, because matching a substring proves only that the arm can print',
