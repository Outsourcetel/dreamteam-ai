...
import { writePerimeterSql, silentNoopWriteSql, sealedEvidenceSql, WRITE_PERIMETER_SQL } from './write-perimeter.mjs';
...
const PROBES = [
  ...
  {
    name: 'evidence-tables-are-sealed',
    why: 'the same question asked of the OTHER role, and nothing else in this repo asks it. Migs 832/835 made verify_de_system refuse to record a check that compared nothing — a digital employee could otherwise bank `matched = true` having read no system of record at all — but BOTH guards live inside the RPC, and `service_role` held INSERT/UPDATE/DELETE/TRUNCATE on de_system_verifications directly, so any edge function writing the table without the RPC bypassed both and could mint evidence for work it never did. Mig 840 revoked that (`revoke all` + `grant select`, the shape mig 744:197 used for discovery_capability_demand_log, which is still the only other write-sealed table here). This arm is what stops it growing back, and it is the LOAD-BEARING half of that change rather than its decoration: measured 2026-08-21, there was NO standing gate on service_role TABLE grants anywhere — this file named service_role exactly once, for a FUNCTION-execute privilege, and all three write-perimeter arms above are `authenticated`-only. ⚠ Mig 716:334 LOOKS like the missing invariant ("TIER B OVER-REVOKED: service_role grants went from % to %") and is NOT — that is a before/after comparison inside 716\'s own DO block, a blast-radius check on 716 itself, and it does not re-run. Without this arm the revoke could be undone by anything, at any time, silently. ⚠ IT DELIBERATELY DOES NOT USE information_schema, unlike its three siblings, and the difference was MEASURED not assumed: information_schema.role_table_grants shows only grants the QUERYING ROLE is party to, so the same question about the same table at the same moment returns 7 rows through db-query.mjs (connects as postgres, the grantor) and 0 rows through the Supabase MCP (connects as supabase_read_only_user). Zero reads as "no grants" — a false ALL-CLEAR that already fooled one audit of this exact table. has_table_privilege gives the same answer whoever asks. THREE FAILURE MODES, not one: a privilege regrowing (A), the sealed table having been renamed or dropped so the seal guards nothing (B, via to_regclass — mig 716:294 learned that `::regclass` raises 42P01 and drops the row instead), and SELECT having been revoked TOO (C — over-revoking is the same defect wearing the opposite mask, cf. mig 643 nearly leaving 11 of 12 workspaces administrable by nobody, and these rows are READ as promotion evidence). The scope is one table on purpose: mig 840\'s header enumerates the fifteen zero-caller candidates and argues why narrowing them to "read as evidence" is a per-table judgment call, which is how a sweep ends up partial while looking complete',
    sql: sealedEvidenceSql(),
  },
  {
