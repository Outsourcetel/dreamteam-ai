    ...
    <div className="flex flex-col h-full bg-slate-900 border border-slate-700 rounded-lg overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-700 bg-slate-800/60">
        <div className="min-w-0">
          ...
          <div className="text-xs text-slate-400 truncate">{subjectLabel}</div>
        </div>
        {onClose && (
          <button onClick={onClose} aria-label="Close"
            className="text-slate-400 hover:text-white text-lg leading-none px-2">×</button>
        )}
      </div>
                  ...
                  return (
                    <div key={c.change_id}
                      className={`mt-2 flex items-start gap-2 text-xs rounded border px-3 py-2 ${
                        isUndone ? 'bg-slate-800/50 border-slate-700 text-slate-500'
