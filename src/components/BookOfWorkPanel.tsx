  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <p className="text-[11px] uppercase tracking-wide text-slate-500">Book of Work — how it finds its own work</p>
        {!adding && (
      ...
      {adding && (
        <div className="mb-3 rounded-lg border border-slate-600 bg-slate-900/70 p-3 space-y-2">
          ...
          {kind === 'date_horizon' && (
            <label className="block text-[11px] text-slate-500">
              ...
              <input value={horizons} onChange={e => setHorizons(e.target.value)} placeholder="90, 60, 30"
                className="mt-1 w-full bg-slate-800 border border-slate-600 text-slate-200 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:border-indigo-500" />
            </label>
          )}
          {kind === 'state_condition' && (
            <div className="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
              <span>When an account's</span>
              <select value={field} onChange={e => setField(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-200 rounded-lg px-2 py-1.5">
                {STATE_FIELDS.map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
              </select>
              <select value={op} onChange={e => setOp(e.target.value)} className="bg-slate-800 border border-slate-600 text-slate-200 rounded-lg px-2 py-1.5">
              ...
              <input value={value} onChange={e => setValue(e.target.value)} placeholder="60"
                className="w-24 bg-slate-800 border border-slate-600 text-slate-200 rounded-lg px-2 py-1.5" />
            </div>
          )}
          {kind === 'metric_threshold' && (
            <div className="flex items-center gap-2 text-xs text-slate-400 flex-wrap">
              ...
              <input value={metricKey} onChange={e => setMetricKey(e.target.value)} placeholder="metric key"
                className="w-32 bg-slate-800 border border-slate-600 text-slate-200 rounded-lg px-2 py-1.5" />
              <select value={metricOp} onChange={e => setMetricOp(e.target.value as 'gt' | 'lt')} className="bg-slate-800 border border-slate-600 text-slate-200 rounded-lg px-2 py-1.5">
          ...
          {kind === 'schedule' && (
            <select value={intervalMin} onChange={e => setIntervalMin(Number(e.target.value))}
              className="w-full bg-slate-800 border border-slate-600 text-slate-200 text-xs rounded-lg px-2 py-1.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => { setAdding(false); resetForm(); setError(null); }} className="text-xs text-slate-500 hover:text-slate-300">Cancel</button>
          </div>
        </div>
      )}
      ...
      {watchers === null ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-700 px-4 py-3 text-xs text-slate-500">
        ...
        <div className="space-y-2">
          {(watchers ?? []).map(w => (
            <div key={w.id} className={`bg-slate-900/50 rounded-lg px-4 py-2.5 ${w.active ? '' : 'opacity-55'}`}>
              <div className="flex items-center gap-2">
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${w.active ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-700 text-slate-400'}`}>
                ...
                <div className="ml-auto flex items-center gap-2 shrink-0">
                  <button onClick={() => void run(() => setWatcherActive(w.id, !w.active))} disabled={busy}
                    className="text-[10px] text-slate-500 hover:text-amber-300">{w.active ? 'pause' : 'resume'}</button>
              ...
              <p className="text-xs text-slate-400 mt-1">{describeWatcher(w)}</p>
              {w.last_run_at && (
