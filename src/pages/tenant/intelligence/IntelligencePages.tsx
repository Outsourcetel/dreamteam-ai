...
function DemoPerformancePage({ setPage }: { setPage: (p: Page) => void }) {
  ...
  return (
    <div className="flex-1 overflow-auto bg-slate-900 p-6">
      ...
      {(() => {
        const inq = des.reduce((s, d) => s + d.tasksThisMonth, 0);
        const acts = des.reduce((s, d) => s + d.actionsTaken, 0);
        const auto = des.reduce((s, d) => s + d.autoActions, 0);
        const sent = des.reduce((s, d) => s + d.sentToHuman, 0);
        const autonomy = acts > 0 ? Math.round(100 * auto / acts) : 0;
        return (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            <StatTile label="Inquiries handled" value={inq.toLocaleString()} sub="this month" />
            <StatTile label="Actions taken" value={acts.toLocaleString()} tone="text-emerald-400" sub={`${auto} on their own · ${acts - auto} after approval`} />
            <StatTile label="Autonomy" value={`${autonomy}%`} tone="text-emerald-400" sub="of actions, no human" />
            <StatTile label="Sent to your team" value={sent.toLocaleString()} tone="text-amber-300" sub="approvals routed to people" />
          </div>
        );
      })()}

      {/* Per-DE outcome cards — lead with what each DE DID */}
