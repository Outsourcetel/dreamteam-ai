...
export default function HumanTasksPage({ setPage }: { setPage: (p: Page) => void }) {
  ...
  const decide = (id: string, status: 'approved' | 'rejected') => {
    ...
    setTasks(prev => {
      ...
      try {
        const decisions: Record<string, TaskStatus> = {};
        next.forEach(t => {
          const seedTask = SEED_TASKS[activeCompanyId].find(s => s.id === t.id);
  ...
  const overdue = pending.filter(t => t.slaRemaining.includes('OVERDUE'));
  const decidedCount = tasks.filter(t => t.status === 'approved' || t.status === 'rejected').length;
  const approvedCount = tasks.filter(t => t.status === 'approved').length;
  const approvalRate = decidedCount > 0 ? Math.round((approvedCount / decidedCount) * 100) : 0;

  const visible = tasks.filter(t =>
    (filter === 'all' || t.type === filter) && (!urgentOnly || t.urgent)
  );
  const selected = tasks.find(t => t.id === selectedId) ?? null;

  const stats = [
    { label: 'Pending', value: String(pending.length), color: pending.length > 0 ? 'text-amber-300' : 'text-emerald-300' },
    { label: 'Overdue', value: String(overdue.length), color: overdue.length > 0 ? 'text-red-300' : 'text-emerald-300' },
    { label: 'Avg response time', value: activeCompanyId === 'tcp' ? '3.2 hrs' : '4.1 hrs', color: 'text-white' },
    ...
    <div className="flex-1 overflow-auto bg-slate-950 p-6">
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs transition-colors ${urgentOnly ? 'bg-red-500/20 text-red-300 border border-red-500/40' : 'bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200'}`}
        >
          ⚑ Urgent only
        </button>
      </div>
          ...
          <div className="col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-5 h-fit sticky top-0">
            ...
            <div className="space-y-3 text-xs">
              ...
              {selected.status === 'pending' ? (
                <div className={`flex items-center justify-between rounded-lg px-3 py-2 ${selected.slaRemaining.includes('OVERDUE') ? 'bg-red-500/10 border border-red-500/30' : 'bg-slate-950'}`}>
