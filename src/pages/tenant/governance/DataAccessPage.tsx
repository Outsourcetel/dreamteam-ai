    ...
    <div className="flex-1 overflow-y-auto bg-slate-900 p-6">
      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-800/50 p-5 mb-6">
        ...
        <p className="text-[11px] text-slate-500 mb-3">
          ...
          No setting at all means <span className="text-slate-300">no access</span> — that is the default for everything.
        </p>
          ...
          <div className="rounded-2xl border border-slate-700 bg-slate-800/50 p-5 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[900px]">
                <thead>
                  <tr>
                    <th className="text-left text-[10px] font-medium text-slate-500 uppercase tracking-wider px-3 py-2">Who</th>
                    {CATEGORIES.map((cat) => (
                      <th key={cat} className="text-left text-[10px] font-medium uppercase tracking-wider px-2 py-2" title={CATEGORY_LABELS[cat]}>
                        <span className={SENSITIVE.has(cat) ? 'text-amber-400' : 'text-slate-500'}>
                ...
                <table className="w-full text-sm min-w-[700px]">
                  <thead>
                    <tr>
                      <th className="text-left text-[10px] font-medium text-slate-500 uppercase tracking-wider px-3 py-2">Who</th>
                      {connectors.map((c) => (
                        <th key={c.id} className="text-left text-[10px] font-medium text-slate-500 uppercase tracking-wider px-2 py-2">
                          <span className="text-slate-300 normal-case">{c.display_name || c.provider}</span>
                          <span className="block text-slate-600 normal-case font-normal">{CAT_SHORT[c.category] ?? c.category}</span>
                        </th>
                      ))}
                    </tr>
                  </thead>
            ...
            {denials.map((d) => (
              <div key={d.id} className="rounded-xl border border-slate-700 bg-slate-900/50 px-3 py-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {d.detail.op && <span className="text-[10px] text-slate-500 font-mono">{d.detail.op}</span>}
                  {d.detail.needed && (
                    <span className="text-[10px] text-slate-500">
                      needed {d.detail.needed}, had {d.detail.has ?? 'no grant'}
                    </span>
                  )}
                  <span className="text-[10px] text-slate-600 ml-auto whitespace-nowrap">{fmtDate(d.created_at)}</span>
                </div>
                <p className="text-[11px] text-slate-500 mt-0.5">{d.action}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-800/50 p-5">
        ...
        <ul className="text-[11px] text-slate-500 space-y-1.5 list-disc pl-4">
          ...
          <li><span className="text-slate-300">Humans are separate:</span> your own clicks in the connector wizard (test, health check, dry run) are governed by workspace roles, not this matrix.</li>
          <li><span className="text-slate-300">Internal knowledge is workspace-wide (v1):</span> documents uploaded to DreamTeam knowledge are readable by every DE and specialist. Named upgrade: per-DE knowledge scopes.</li>
          <li><span className="text-slate-300">Write-back grants don't skip approvals:</span> a write still goes through the existing human gates — the grant only decides whether the request may exist at all.</li>
        </ul>
      </div>
    </div>
  );
