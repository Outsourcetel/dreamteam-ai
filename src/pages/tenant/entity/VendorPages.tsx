  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-dt-support text-sm mt-1">One relationship per vendor, end-to-end</p>
      ...
      {/* Journey bar */}
      <div className="mb-8">
        <h2 className="text-xs font-semibold text-dt-support uppercase tracking-wide mb-3">Vendor journey</h2>
        <div className="flex items-stretch gap-1 overflow-x-auto pb-2">
          {stages.map((s, i) => (
            <React.Fragment key={s.page}>
              <button
                onClick={() => setPage(s.page)}
                className="flex-shrink-0 w-48 text-left rounded-xl p-3.5 border border-dt-border bg-dt-card hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all group"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 rounded-2xl border border-dt-border bg-dt-card p-5">
          ...
          <div className="overflow-x-auto rounded-xl border border-dt-border">
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {evals.map((e, i) => (
                  <tr key={e.vendor} className={`border-b border-dt-border hover:bg-dt-panel transition-colors ${i === evals.length - 1 ? 'border-b-0' : ''}`}>
                    <td className={`${td} font-medium text-white`}>{e.vendor}</td>
                    <td className={`${td} text-dt-support text-xs`}>{e.category}</td>
                    <td className={td}><span className={`text-xs px-2 py-0.5 rounded-full ${evalStageBadge(e.stage)}`}>{e.stage}</span></td>
                    <td className={`${td} text-xs ${e.score != null ? (e.score >= 85 ? 'text-emerald-300' : 'text-dt-support') : 'text-dt-faint'}`}>{e.score != null ? `${e.score}/100` : '—'}</td>
                    <td className={`${td} text-dt-support text-xs`}>{e.owner} <span className="text-dt-faint">(human)</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-2xl border border-dt-border bg-dt-card p-5">
          ...
          <div className="space-y-3">
            {rfps.map(r => (
              <div key={r.title} className={`rounded-xl border p-3 ${r.tone === 'closing' ? 'border-amber-500/30 bg-amber-500/5' : 'border-dt-border bg-dt-card'}`}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-white">{r.title}</span>
                  <span className={`text-[10px] px-1.5 py-0.5 rounded ${r.tone === 'closing' ? 'bg-amber-500/15 text-amber-300' : r.tone === 'draft' ? 'bg-dt-panel text-dt-support' : 'bg-indigo-500/15 text-indigo-300'}`}>{r.status}</span>
                </div>
                <p className="text-xs text-dt-muted">Due {r.due} · {r.responses}</p>
              </div>
            ))}
          </div>
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {contracts.map((c, i) => {
                  ...
                  return (
                    <tr key={c.vendor} className={`border-b border-dt-border transition-colors ${warn ? 'bg-amber-500/5 hover:bg-amber-500/10' : 'hover:bg-dt-panel'} ${i === contracts.length - 1 ? 'border-b-0' : ''}`}>
                      <td className={`${td} font-medium ${warn ? 'text-amber-200' : 'text-white'}`}>{c.vendor}</td>
                      <td className={`${td} text-dt-support text-xs`}>{c.type}</td>
                      <td className={`${td} text-dt-support`}>{c.valuePerYear}</td>
                      <td className={`${td} text-dt-support text-xs whitespace-nowrap`}>{c.start}</td>
                      <td className={`${td} text-xs whitespace-nowrap ${warn ? 'text-amber-300' : 'text-dt-support'}`}>{c.expiry}</td>
                      <td className={`${td} text-xs ${c.autoRenew ? 'text-emerald-400' : 'text-dt-muted'}`}>{c.autoRenew ? 'Yes' : 'No'}</td>
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/5 p-5 h-fit">
          ...
          <p className="text-xs text-dt-muted mb-4">{expiring.length} contract{expiring.length === 1 ? '' : 's'} within 60 days</p>
          <div className="space-y-3">
            {expiring.map(c => (
              <div key={c.vendor} className="rounded-xl border border-dt-border bg-dt-card p-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-white">{c.vendor}</span>
                  <span className="text-xs text-amber-300">{c.expiry}</span>
                </div>
                <p className="text-xs text-dt-muted mb-2">{c.type} · {c.valuePerYear}/yr · {c.autoRenew ? 'auto-renews' : 'no auto-renew'}</p>
                <button className="text-xs px-2.5 py-1 rounded-lg bg-dt-panel text-dt-support hover:bg-dt-panel border border-dt-border-strong transition-colors">
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {cards.map(c => (
          <div key={c.label} className="bg-dt-card border border-dt-border rounded-xl p-4">
            <p className="text-[11px] uppercase tracking-wide text-dt-muted mb-1">{c.label}</p>
            <p className={`text-xl font-bold ${c.color}`}>{c.value}</p>
            <p className="text-xs text-dt-muted mt-0.5">{c.sub}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-dt-border bg-dt-card p-5">
        ...
        <div className="overflow-x-auto rounded-xl border border-dt-border">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {reviews.map((r, i) => (
                <tr key={r.vendor} className={`border-b border-dt-border hover:bg-dt-panel transition-colors ${i === reviews.length - 1 ? 'border-b-0' : ''}`}>
                  <td className={`${td} font-medium text-white`}>{r.vendor}</td>
                  <td className={`${td} text-dt-support text-xs`}>{r.quarter}</td>
                  <td className={`${td} text-dt-support text-xs whitespace-nowrap`}>{r.date}</td>
                  <td className={`${td} text-dt-support text-xs`}>{r.owner} <span className="text-dt-faint">(human)</span></td>
