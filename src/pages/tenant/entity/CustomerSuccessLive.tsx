...
function StatusChip({ status }: { status: CustomerAccount['status'] }) {
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap ${
      ...
      : status === 'at_risk' ? 'bg-dt-danger-soft text-dt-danger'
      : 'bg-dt-ok-soft text-dt-ok'
      ...
      <div className="space-y-1">
        {rows.map(r => (
          <div key={r.label} className="flex items-center justify-between text-[11px]">
            ...
            <span className={r.penalty > 0 ? 'text-dt-danger font-medium' : 'text-dt-faint'}>
...
function ContactsPanel({ accountId, accountName }: { accountId: string; accountName: string }) {
  ...
  return (
    <div className="rounded-xl border border-dt-border bg-dt-inset p-3 mb-4">
      ...
      {err && <p className="text-[11px] text-dt-danger mb-2">{err}</p>}
            ...
            <div key={c.id} className="flex items-start justify-between gap-2 py-1 border-b border-dt-border last:border-0">
              <div className="min-w-0">
                <p className="text-xs text-dt-body truncate">
                  ...
                  {c.is_primary && <span className="ml-1.5 text-[10px] px-1.5 py-0.5 rounded bg-dt-accent-soft text-dt-accent-text">primary</span>}
              ...
              {canEdit && (
                <div className="flex gap-1 shrink-0">
                  ...
                  <button onClick={() => void remove(c)} className="text-[11px] px-1.5 py-0.5 rounded text-dt-muted hover:text-dt-danger transition-colors">Remove</button>
                </div>
              )}
            </div>
          ))}
...
function AccountDrawer({ account, onClose, onChanged }: {
  ...
  return (
    <Drawer title={account.name} onClose={onClose}>
        ...
        {err && <div className="mb-3 rounded-xl border border-dt-danger-border bg-dt-danger-soft px-3 py-2 text-xs text-dt-danger">{err}</div>}
            ...
            <Section title={`Open tickets (${signals.openTickets.length})`}>
              {signals.openTickets.length === 0 ? <Empty text="No open tickets." /> : signals.openTickets.map(t => (
                <div key={t.id} className="flex items-center justify-between gap-2 text-xs py-1">
                  ...
                  <span className={`px-1.5 py-0.5 rounded whitespace-nowrap ${t.status === 'escalated' ? 'bg-dt-danger-soft text-dt-danger' : 'bg-dt-panel text-dt-support'}`}>{t.priority} · {t.status}</span>
                </div>
              ))}
            </Section>
            <Section title={`Overdue invoices (${signals.overdueInvoices.length})`}>
              {signals.overdueInvoices.length === 0 ? <Empty text="No overdue invoices." /> : signals.overdueInvoices.map(inv => (
                <div key={inv.id} className="flex items-center justify-between text-xs py-1">
                  ...
                  <span className="text-dt-danger">due {inv.due_date}</span>
                </div>
              ))}
            </Section>
            ...
            {((account.attributes as Record<string, unknown> | null)?.next_step || accountActs.length > 0) && (
              <Section title="Employee write-backs">
                {(account.attributes as Record<string, unknown> | null)?.next_step ? (
                  <p className="text-xs py-1"><span className="text-[10px] px-1.5 py-0.5 rounded bg-dt-accent-soft text-dt-accent-text mr-1.5">next step</span>
              ...
              {signals.atRiskFires.length === 0 ? <Empty text="No event-trigger fires for this account." /> : signals.atRiskFires.map(f => (
                <div key={f.id} className="flex items-center justify-between gap-2 text-xs py-1">
                  ...
                  <span className={`px-1.5 py-0.5 rounded whitespace-nowrap ${
                    f.status === 'started' ? 'bg-dt-ok-soft text-dt-ok'
                    : f.status === 'skipped_dedup' ? 'bg-dt-panel text-dt-muted'
                    : f.status === 'error' ? 'bg-dt-danger-soft text-dt-danger' : 'bg-dt-accent-soft text-dt-accent-text'
        ...
        <div className="rounded-xl border border-dt-border bg-dt-inset p-3 mt-4">
          ...
          {runMsg && <p className="text-[11px] text-dt-ok mt-2">✓ {runMsg}</p>}
        </div>
    </Drawer>
  );
}
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
            {[
              ...
              { label: 'At risk', value: String(stats.atRisk), color: stats.atRisk > 0 ? 'text-dt-danger' : 'text-dt-ok' },
              { label: 'Avg health', value: String(stats.avgHealth), color: healthText(stats.avgHealth) },
              { label: `${vocab.value_metric} at risk`, value: fmtMoneyK(stats.arrAtRisk), color: stats.arrAtRisk > 0 ? 'text-dt-danger' : 'text-dt-support' },
            ].map(s => (
                  ...
                  {accounts.map((a, i) => {
                    ...
                    return (
                      <tr key={a.id} onClick={() => setDrawerAccount(a)}
                        className={`border-b border-dt-border transition-colors cursor-pointer ${risk ? 'bg-dt-danger-soft hover:brightness-110' : 'hover:bg-dt-panel'} ${i === accounts.length - 1 ? 'border-b-0' : ''}`}>
                        <td className={`${td} font-medium ${risk ? 'text-dt-danger' : 'text-dt-title'}`}>{a.name}</td>
